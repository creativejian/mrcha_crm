...
- **⚠️ 채팅 system 문구는 앱과의 문자열 계약(2026-07-10 실측)**: `client/src/data/chat.ts`의 `CHAT_SYSTEM_MSG_TAKEOVER`·`CHAT_SYSTEM_MSG_RETURN`을 앱이 **접두사 매칭**으로 읽어 상담 상태를 복원한다(`mr-cha-app/lib/core/constants/handoff_system_messages.dart:20-21` `startsWith`). 한 글자만 바꿔도 앱 복원이 **예외 없이 조용히** 깨지고, 리포지토리가 달라 타입·테스트가 못 잡는다. 변경 시 앱 팀 사전 공지 필수. (메타 필드 신호로 전환하기로 합의 — 그때까지 이 상수가 계약.)
