...
- DB — vehicle catalog (mirror): the car catalog (brands/models/trims/options/colors) is mirrored from the Mr.Cha app Supabase into a **`catalog` schema** (7 tables, ~29k rows, imported 2026-06-14). It is isolated in `catalog` (not `public`) so drizzle — which only manages `public` — can never DROP it via `db:push`. Read-only mirror with `deleted_at` soft-delete; sync via PostgREST is not yet built. Full design/decisions: `ref/vehicle-mirror-db.md`. Import assets in `ref/db_import/` (gitignored). Master URL/key in `.env.local` (`MRCHA_MASTER_*`).
- DB — CRM domain: `public` still has **0 tables / 0 migrations**. The Drizzle schema (`customers`, `consultations`) is only defined in `src/db/schema.ts`; there is no `drizzle/` folder and the app has no DB connection layer yet. `DATABASE_URL` lives in `.env.local` (the docs still reference `.dev.vars`).
