...
- **③ CI 도입이 감사보다 값어치 있다** → ✅ **완료(2026-07-22, `.github/workflows/ci.yml`)**. push(main)·PR마다 **8단계**가 자동으로 돈다 — **typecheck · lint · knip · format:check · test:unit · test:pure · build · test:edge**(Deno, Edge Function 26건). 앞 단계가 실패해도 나머지를 계속 돌려 한 번에 전부 보고한다.
  - **잡 이름 = `typecheck · lint · knip · format · unit · pure · build · edge`**(2026-07-25 `pure` 추가·2026-07-23 정정). 구 이름은 `typecheck · lint · unit · build`라 4종만 말했고, `gh pr checks` 출력에 그 문자열만 보이는 탓에 **knip·format:check·edge가 사람 눈에서 가려졌다** — 그 이름과 당시 스테일했던 아래 서술이 겹쳐 #333에서 로컬 knip을 건너뛰었고 unused export 1건으로 CI가 빨개졌다. ⚠️ **step을 추가·제거하면 잡 이름도 함께 고친다**(워크플로우 주석에 같은 경고를 박아뒀다).
...
- **⚠️ 금융사 SSOT는 파트너 목록의 하드코딩 미러다(2026-07-23)**: `SOLUTION_LENDERS`(`client/src/lib/solution-quote.ts`)는 제프 금융사 목록을 손으로 옮긴 상수다. **실시간 fetch로 바꾸지 말 것** — ①`SolutionLenderCode`가 이 상수에서 파생된 **컴파일타임 타입**이라 런타임 값으로는 못 만든다(딜러·지원집합 호출의 타입 안전이 여기서 나온다) ②`label`은 저장 견적(`crm.quotes.scenarios[].lender`)에 박히는 **데이터 계약**이다 ③`CRM_EXTRA_LENDERS`로 CRM이 **상위집합**을 소유하는 설계라 파트너 목록으로 덮으면 그 확장점이 죽는다 ④파트너가 죽어도 금융사 드롭다운은 떠야 한다(계산·딜러·매트릭스만 파트너 의존). 금융사 추가/삭제는 **상수 한 줄 + 배포**가 의도된 유지 경로다. **드리프트 그물 2겹**: ⓐ런타임 — 워크벤치가 support-matrix를 받을 때 `lenderCode` 집합을 양방향 대조해 어긋나면 콘솔 경고 1회(화면 동작은 불변, fail-open 유지) ⓑ수동 — **`bun run check:lenders`**(파트너 직접 조회, 어긋나면 exit 1). 판정은 순수 `detectLenderDrift`(solution-quote) 한 벌을 둘이 공유한다. ⚠️ **CI에는 넣을 수 없다** — CI는 파트너 네트워크·시크릿이 없다(env 없이 도는 게 CI 설계 전제). **축은 3개** — 추가(`onlyPartner`) · 삭제(`onlyCrm`) · **개명(`renamed`)**. 개명 축은 제프가 `lenderName`을 실어 주면서 열렸다(요청 `ref/2026-07-23-jeff-lender-name-request.md` / 회신 `…-reply.md` — **파트너 배포 완료·실기 확인**). ⚠️ **`lenderName`이 없는 응답에서는 개명 축만 조용히 건너뛴다**(구 캐시·롤백 대비 — 없으면 8사 전부가 "개명됨"으로 오탐한다). ⚠️ 런타임 경고는 파싱 Map이 아니라 **raw 응답**을 봐야 한다(`lenderName`은 게이트 데이터가 아니라 `parseSupportMatrix` 결과에 남지 않는다). **개명이 잡히면 label만 갱신하면 된다** — code가 그대로면 계산·딜러·지원집합은 무영향이고, 과거 견적의 구 표기는 "구 어휘 표시 유지" option이 담당하므로 **소급 갱신 불필요**. 🔵 **예고된 변경 1건**: 파트너가 **하나캐피탈** 추가를 검토 중(운용리스 엔진 빌드 완료·배선 보류). 배선되면 support-matrix에 자동으로 행이 생기지만 **우리는 `code` 유니온이 컴파일타임 타입이라 자동 반영이 안 된다** — 파트너가 착수 전 통보하기로 했고, 그때 `SOLUTION_LENDERS`에 `{ code, label }` 한 줄을 추가한다(가드도 `onlyPartner`로 잡는다). 파트너 쪽도 대칭 가드를 세웠다(라우트 테스트에 8사 표시명 리터럴 → 그쪽이 개명·추가·삭제하면 그쪽 CI가 먼저 빨개진다).
- **⚠️ `updated_at`은 DB 시계로만 찍는다(2026-07-23 `#334`·`#335`)**: 스키마의 `defaultNow()` 때문에 **INSERT는 이미 DB 시계**인데 UPDATE만 `new Date()`(앱 시계)로 찍고 있었다 → 앱↔DB 시계가 어긋난 만큼 **갱신할 때마다 스탬프가 과거로 되돌아간다**(로컬 실측: 앱이 2.08초 뒤처져 `customer_deliveries` upsert 12/12에서 `updated_at < created_at`). 반드시 **`sql\`now()\``**. 왜 치명적인가 = `customers.updated_at`·`customer_deliveries.updated_at`이 `staffActivityAt`(`queries/activity.ts`)의 greatest() 항목이라 **목록 "마지막 활동"·무활동 일수·`stale_customers`/`delivery_risk`·수동 관리 상태(스누즈) 유효 판정**을 전부 결정한다 — 뒤로 가면 "방금 저장했는데 더 오래 방치"로 읽힌다. **스누즈는 더 미묘하다**: 유효 규칙이 `manage_status_at >= staffActivityAt`인데 그 greatest에는 자식 테이블 `created_at`(DB 시계)이 들어가므로, `manage_status_at`만 앱 시계면 **켜자마자 만료**될 수 있다. 그래서 `updateCustomer`는 두 스탬프를 **각각 인라인 `sql\`now()\``**로 찍는다(한 statement 안의 `now()`는 몇 번을 써도 같은 값 — 실 DB 테스트로 잠금). ⚠️ **테스트에서 스탬프 전진을 단언할 땐 JS Date로 꺼내 비교하지 말 것** — ms 절삭이라 빠른 연속 호출에서 거짓 실패하고(구 `not.toBe`가 "전체 실행에서만 실패하는 플레이크"의 정체였다), 더 나쁘게는 **스큐가 클수록 잘 통과해 결함을 가린다.** DB 안에서 `updated_at > created_at`으로 비교한다(timestamptz = 마이크로초). 회귀 그물 = `src/db/updated-at-clock-guard.test.ts`(소스 스캔 tripwire — 정규식을 좁혀 우회하지 말고 정당한 예외는 `ALLOW`에 사유와 함께 등록).
