...
- DB — vehicle catalog (mirror): the car catalog (brands/models/trims/options/colors) is mirrored from the Mr.Cha app Supabase into a **`catalog` schema** (7 tables, ~29k rows, imported 2026-06-14). It is isolated in `catalog` (not `public`) so drizzle — which only manages `public` — can never DROP it via `db:push`. Read-only mirror with `deleted_at` soft-delete. A read-only query API is exposed at `/api/vehicles` (brands/models/trims + trim detail, via `src/db/client.ts` + `src/db/queries/vehicles.ts` + `src/routes/vehicles.ts`), and wired into the Kim quote workbench via `client/src/components/VehiclePicker.tsx` (brand→model→trim dropdown, pure fetch). Selected vehicle is not yet fed into price/option calc; PostgREST sync is not yet built. Full design/decisions: `ref/vehicle-mirror-db.md`. Import assets in `ref/db_import/` (gitignored). Master URL/key in `.env.local` (`MRCHA_MASTER_*`).
- DB — CRM domain: `public` still has **0 tables / 0 migrations**. The Drizzle schema (`customers`, `consultations`) is only defined in `src/db/schema.ts`; there is no `drizzle/` folder. (A DB connection layer — `src/db/client.ts` — now exists and powers the catalog read API.) `DATABASE_URL` lives in `.env.local` (the docs still reference `.dev.vars`).
