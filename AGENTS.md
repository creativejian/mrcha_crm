...
- **⚠️ 채팅 system 문구는 앱과의 문자열 계약(2026-07-10 실측 → 2026-07-11 범위 정밀화)**: `client/src/data/chat.ts`의 `CHAT_SYSTEM_MSG_TAKEOVER`·`CHAT_SYSTEM_MSG_RETURN` 문구를 앱 `handoff_system_messages.dart`의 **6개 함수**(`isExternalNotification`·`isHandoffSeparator`·`isAppHiddenInactivityWarning`·`separatorDisplayText`·`toastText`·`toastDuration`)가 startsWith/exact 매칭 키로 소비한다. 핸드오버 **상태**는 `chat_sessions.mode` 기반이라 문구와 무관하지만(구 서술 "상담 상태를 복원한다"는 부정확), **알림 dispatch·핸드오버 구분선·토스트 문구/지속시간**이 문구에 걸려 있어 한 글자만 바꿔도 그 UI가 **예외 없이 조용히** 깨진다. 리포지토리가 달라 타입·테스트가 못 잡는다. 변경 시 앱 팀 사전 공지 필수. **해소 진행 중(2026-07-11 합의)**: `public.chat_messages.metadata` jsonb 신설(앱 마이그) — 슬라이스 1=앱 자기생성 3종 `system_kind` 부착(진행 중), 슬라이스 2=CRM `insertSystemMessage`(`client/src/lib/chat.ts`)가 `handoff_takeover`/`handoff_return` kind 부착 + 앱 6함수 flag-first 전환(선행=앱팀 생성 주체 매트릭스 공유). **슬라이스 2 완료 전까지 이 상수가 계약.** kind 네임스페이스 확정치·앱 내부 문구 매칭 지도(AI 응답 2곳 포함)는 협의 기록 참조.
