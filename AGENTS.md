...
- **⚠️ 운영 알림 트리거 테이블(2026-07-09 사고)**: `DATABASE_URL`이 공유 master라 `public.consultations`(트리거 `on_consultation_created`)·`public.advisor_quotes`(트리거 `on_advisor_quote_sent`)에 INSERT/UPDATE하면 `net.http_post` → Edge Function → **운영 디스코드 알림·관리자 FCM 푸시가 실제로 나간다**(테스트가 행을 지워도 트리거는 이미 발화). `test:server`의 `PUSH_NOTIFY=off`는 CRM 앱 코드 경로만 막고 **DB 트리거는 못 막는다**. 그래서 그 테이블에 쓰는 테스트는 `src/test-utils/notify-gate.ts`로 **기본 skip**이다. **`NOTIFY_TRIGGER_TESTS=on`(=`bun run test:server:notify`)은 켜지 말 것** — 실제 알림이 나간다. 앱 팀이 세 트리거에 `current_setting('app.skip_notify',true)='on' AND session_user='postgres'` 가드를 배포 중이며, 배포되면 픽스처를 `db.transaction()`+`set_config('app.skip_notify','on',true)`로 감싸고 게이트를 제거한다(`db.insert()` 단독은 자동커밋이라 `SET LOCAL`이 안 걸린다). `application_name` 방식은 Supavisor가 덮어써 불가(실측). `session_user`는 pooler·SECURITY DEFINER 경유에도 `postgres`로 유지(실측).
