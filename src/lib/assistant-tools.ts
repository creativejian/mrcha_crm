...
const TOOL_DECLARATION_DEFS: Record<AssistantToolKey, ToolDeclarationDef> = {
  ...
  search_customers: {
    description: "조건으로 고객을 검색해 목록을 조회한다 — 각 고객의 연락처(주 번호·추가 연락처)·상담경로·진행 상태·구매방식을 함께 반환한다. 특정 고객의 연락처·번호처럼 **고객 속성**을 묻는 질문(예 '제임스 연락처?', '김지안 번호 뭐야')도 그 이름을 name에 넣어 이 함수를 호출하면 답할 수 있다. 검색 **필터**는 이름·진행 상태·구매방식·상담경로(유입 경로)·내 담당 여부 **다섯 가지뿐**이다 — 관심 차종(예 '마이바흐 관심 고객')·예산·지역처럼 이 필터에 없는 조건으로 고객을 찾는 질문에는 이 함수를 호출하지 마라 — 아래 파라미터를 임의로 채워 부르면 질문과 무관한 목록이 나온다.",
    parameters: {
      type: "object",
      properties: {
