...
export const ROUTER_SYSTEM_PROMPT = [
  ...
  // 위 억제 문구와 양립하는 반대 방향 안내(2026-07-23) — 실기에서 `"제임스 연락처?"`가 어미에 따라
  // 4가지로 갈렸다. 필터 목록이 **입력(검색 조건)** 규정인데 모델이 **출력(답에 담길 내용)** 제한으로도
  // 읽어, 이름으로 찾을 수 있는 질문까지 '해당 없음'으로 흘렸다. 그 경계를 말로 못 박는다.
  "지원하는 필터는 **검색 조건**을 말하는 것이지 답에 담길 내용을 제한하지 않습니다 — 특정 고객의 연락처·번호를 물으면(예: '제임스 연락처?', '김지안 번호 뭐야') 그 이름을 search_customers의 name에 넣어 호출하세요(조회 결과에 연락처가 함께 들어 있습니다).",
  "함수로 답할 수 없는 질문(잡담·일반 지식·특정 고객의 서술형 맥락 — 예: '○○이 원하는 조건', '상담 분위기 어땠어')이면 함수를 호출하지 말고 '해당 없음'이라고만 답하세요.",
].join("\n");
