...
test("embeddingContentHash: 같은 문자열 같은 해시, 다르면 다름", () => {
  expect(embeddingContentHash("a")).toBe(embeddingContentHash("a"));
  expect(embeddingContentHash("a")).not.toBe(embeddingContentHash("b"));
});

// 스킴 잠금(배치 14 K1-b) — 위 두 단언은 **자기참조라 salt를 통째로 걷어내도 GREEN**이다(스팟 3회 실증:
// 모델명 접두를 지우는 변이가 전 스위트를 통과했다). 모델명 salt는 구·신 모델 벡터 혼입을 막는 **단일
