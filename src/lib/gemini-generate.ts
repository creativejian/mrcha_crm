...
// 바뀐다**(`assistant-tool-router.ts`가 같은 상수를 쓴다). **골든 테스트는** 라우터를 페이크로 주입하고,
// 라우트 테스트는 실 라우터를 부르되 결과에 의존하지 않으므로(업스트림을 죽여도 전량 통과 — 폴백이 삼킨다)
// 어느 쪽도 이 변화를 못 잡는다(배치 14 K2-d · 배치 15 M7) — 모델을 올릴 때는 **실기 골든 4종**을 눈으로 확인할 것:
//   "마이바흐 관심 고객이 누구야?" → 라우팅 없음(RAG로 답) · "김지안 견적 몇 개야?" → customer_quotes
//   "앱으로 들어온 고객 알려줘" → search_customers{source:"앱"} · "오늘 점심 뭐 먹을까?" → 범위 밖 안내
// ⚠️ **골든 4종만으로는 부족하다**(배치 15 실측): 이 4종은 `3.1-flash-lite`와 `3.5-flash-lite`가 **똑같이
// 4/4 통과**해 두 모델을 구분하지 못했다. 회귀 감시로는 유효하나 "상향이 라우팅을 바꿨다"의 증거는 아니다.
//
...
// 임베딩 모델(`gemini-embed.ts`)과 달리 **앱과 달라도 무방**하다(계약 아님). ⚠️ 근거는 "저장물이 없어서"가
// 아니다(배치 15 M4 정정 — 저장처 2종 실재: `customers.ai_summary`·`crm.assistant_messages`). 둘 다 crm
// 스키마 CRM 소유라 **앱과 공유되지 않는다**는 것이 근거다. 저장물이 있으므로 이 상수를 바꾸면 기존 힌트는
// 그대로 남는다 — 문구를 갈아끼우려면 `ai_summary_source_hash` 클리어 + 백필이 필요하다(`ai-hint.ts` 참조).
