...
// 두 번째 문장 = 표기 유지(2026-07-23 prod 실기 후속). 근거·조회 결과를 이미 하이픈 포맷으로
// 주는데도(`formatPhone` — 화면과 같은 표기) 모델이 **가끔 digits로 정규화한다**(prod 9회 중 1회
// 실측: `01095880812`). 자료 형식만으로는 부족해 지시를 한 겹 더 얹는다.
// ⚠️ 이것도 보증이 아니다 — 확정적으로 만들려면 클라에서 답변 텍스트를 후처리해야 하는데,
// 그건 "AI가 쓴 답을 사후 변조"라는 다른 성격의 결합이라 표기 흔들림 정도에는 과하다고 보고 기각했다.
const CONTACT_DISCLOSURE_RULE =
  "연락처(전화번호)는 질문이 연락처를 물었을 때만 답에 포함하세요 — 근거나 조회 결과에 함께 실려 있어도 다른 질문의 답에는 쓰지 마세요. 답할 때는 근거·조회 결과에 적힌 표기(하이픈 포함)를 그대로 옮기고 숫자만 남기지 마세요.";
