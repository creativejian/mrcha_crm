...
test("generateAnswer: system+user 프롬프트 전송, 텍스트 파싱", async () => {
  ...
  const out = await generateAnswer("SYS", "USER", TARGET, { fetchImpl: fakeFetch });
...
test("generateAnswer: 실패 응답은 throw", async () => {
  ...
  await expect(generateAnswer("s", "u", TARGET, { fetchImpl: fakeFetch })).rejects.toThrow();
});
...
test("generateAnswerStream: alt=sse 라인에서 텍스트 청크를 순서대로 yield", async () => {
  ...
  const out: string[] = [];
  for await (const c of generateAnswerStream("SYS", "USER", TARGET, { fetchImpl: fakeFetch })) out.push(c);
...
test("generateAnswerStream: 청크 경계가 라인 중간에서 갈라져도 파싱", async () => {
  ...
  const out: string[] = [];
  for await (const c of generateAnswerStream("s", "u", TARGET, { fetchImpl: fakeFetch })) out.push(c);
...
test("generateAnswerStream: HTTP 실패는 throw(스트림 시작 전)", async () => {
  ...
  const gen = generateAnswerStream("s", "u", TARGET, { fetchImpl: fakeFetch });
...
test("generateAnswerStream: 전달한 signal이 업스트림 fetch에 배선된다", async () => {
  ...
  const out: string[] = [];
  for await (const c of generateAnswerStream("s", "u", TARGET, { fetchImpl: fakeFetch, signal: ac.signal })) out.push(c);
...
test("generateAnswerStream: 소비자 조기 break 시 업스트림 스트림을 cancel", async () => {
  ...
  const out: string[] = [];
  for await (const c of generateAnswerStream("s", "u", TARGET, { fetchImpl: fakeFetch })) {
    out.push(c);
