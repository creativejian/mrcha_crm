...
test("embedTexts: 빈 배열은 API 호출 없이 []", async () => {
  const out = await embedTexts([], TARGET, "RETRIEVAL_QUERY", (() => { throw new Error("호출되면 안 됨"); }) as unknown as typeof fetch);
  expect(out).toEqual([]);
});

test("embedTexts: 실패 응답은 throw", async () => {
  const fakeFetch = (async () => new Response("boom", { status: 500 })) as unknown as typeof fetch;
  await expect(embedTexts(["a"], TARGET, "RETRIEVAL_QUERY", fakeFetch)).rejects.toThrow();
});
...
test("embedTexts: 100개 초과는 배치로 쪼개 호출하고 순서를 보존해 합친다", async () => {
  ...
  const out = await embedTexts(texts, TARGET, "RETRIEVAL_DOCUMENT", fakeFetch);
