...
// phone 배타 CHECK(customers_phone_app_exclusive_check, 마이그 0034)의 23514 매핑(배치 8 C#5) —
// PATCH 409 게이트 통과 후 동시 link가 끼어든 TOCTOU 최후 방어선인데 generic 문구("허용되지 않는
// 값입니다")로는 사유가 불투명했다. drizzle 0.44 래핑(cause 체인)을 합성해 collectErrorText 경유까지 잠근다.
test("run: phone 배타 CHECK 위반 → 앱 연결 고객 번호 문구(generic 23514 문구 아님)", async () => {
  const app = new Hono();
  app.post("/t", (c) =>
    run(c, () => {
      throw new Error('Failed query: update "crm"."customers" set "phone" = $1', {
        cause: Object.assign(
          new Error('new row for relation "customers" violates check constraint "customers_phone_app_exclusive_check"'),
          { code: "23514", constraint_name: "customers_phone_app_exclusive_check" },
        ),
      });
    }),
  );
  const res = await app.request("/t", { method: "POST" });
  expect(res.status).toBe(500);
  expect(await res.json()).toEqual({ error: "앱 연결 고객의 번호는 저장할 수 없습니다." });
});
