...
// 판매사(dealer_name, 마이그 0033 — T2) 왕복 — cm/ag 왕복(#264 위 테스트) 미러. plain 이름 저장
// (계산기 `lenderCode::dealerName` 합성과 다름 — 금융사는 lender 컬럼이 이미 보유). 미전송 = null
// 왕복(수기/비제휴 시나리오·구 클라 payload 하위 호환).
test("시나리오 판매사(dealer_name) 왕복(저장→조회) — 저장값 보존 + 미전송 null", async () => {
  const { token, keyResolver, issuer } = await makeTestAuth("admin");
  const app = createApp({ keyResolver, issuer });
  const h = { Authorization: `Bearer ${token}`, "Content-Type": "application/json" };
  const list = (await (await app.request("/api/customers", { headers: { Authorization: `Bearer ${token}` } })).json()) as Array<{ id: string }>;
  ...
  try {
    const created = await app.request(`/api/customers/${cid}/quotes`, {
      ...
      body: JSON.stringify({
        entryMode: "manual", status: "작성중",
        scenarios: [
          { scenarioNo: 1, isSaved: true, purchaseMethod: "운용리스", lender: "BNK캐피탈", monthlyPayment: "1500000", termMonths: 60, dealerName: "도이치모터스" },
          // 필드 미전송(비제휴/수기·구 클라 payload) — null로 왕복해야 한다.
          { scenarioNo: 2, isSaved: true, purchaseMethod: "운용리스", lender: "신한카드", monthlyPayment: "1600000" },
        ],
      }),
    });
    ...
    const byNo = [...detail.quotes.find((x) => x.id === quoteId)!.scenarios]
      .sort((a, b) => ((a.scenarioNo as number | null) ?? 0) - ((b.scenarioNo as number | null) ?? 0));
    expect(byNo[0].dealerName).toBe("도이치모터스");
    expect(byNo[1].dealerName).toBeNull();
  } finally {
    // 공유 master DB라 항상 정리(scenarios는 ON DELETE CASCADE).
    if (quoteId) await getDefaultDb().delete(quotes).where(eq(quotes.id, quoteId));
  }
...
// ── 수동 관리 상태 영속(이사님 2026-07-13 ⑦-①: 영속화 + "다음 실활동까지 유효" 스누즈) ──────────
// 스누즈 계약의 핵심 = 설정 직후엔 유효해야 한다: PATCH가 updated_at을 bump하므로(=staffActivityAt 갱신)
// manage_status_at을 updated_at과 **같은 now**로 찍지 않으면 저장하자마자 만료되는 자기모순이 난다.
test("PATCH manageStatus: 저장 + manage_status_at=updated_at 동일 스탬프(설정 직후 스누즈 유효), null=클리어", async () => {
  const { token, keyResolver, issuer } = await makeTestAuth("admin");
  const app = createApp({ keyResolver, issuer });
  const h = { Authorization: `Bearer ${token}`, "Content-Type": "application/json" };
  const db = getDefaultDb();
  const [c] = await db.insert(customers)
    .values({ customerCode: `CU-MGST-${crypto.randomUUID().slice(0, 8)}`, name: "관리상태영속테스트" })
    .returning({ id: customers.id });
  try {
    const res = await app.request(`/api/customers/${c.id}`, { method: "PATCH", headers: h, body: JSON.stringify({ manageStatus: "지연" }) });
    expect(res.status).toBe(200);
    const [row] = await db
      .select({ manageStatus: customers.manageStatus, manageStatusAt: customers.manageStatusAt, updatedAt: customers.updatedAt })
      .from(customers).where(eq(customers.id, c.id));
    expect(row.manageStatus).toBe("지연");
    expect(row.manageStatusAt?.getTime()).toBe(row.updatedAt.getTime());

    const clear = await app.request(`/api/customers/${c.id}`, { method: "PATCH", headers: h, body: JSON.stringify({ manageStatus: null }) });
    expect(clear.status).toBe(200);
    const [cleared] = await db
      .select({ manageStatus: customers.manageStatus, manageStatusAt: customers.manageStatusAt })
      .from(customers).where(eq(customers.id, c.id));
    expect(cleared.manageStatus).toBeNull();
    expect(cleared.manageStatusAt).toBeNull();
  } finally {
    await db.delete(customers).where(eq(customers.id, c.id));
  }
});

test("PATCH manageStatus: 어휘 밖 값 → 400(zod 닫힌 집합)", async () => {
  const { token, keyResolver, issuer } = await makeTestAuth("admin");
  const app = createApp({ keyResolver, issuer });
  const res = await app.request("/api/customers/00000000-0000-0000-0000-000000000000", {
    method: "PATCH",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ manageStatus: "없는상태" }),
  });
  expect(res.status).toBe(400);
});
