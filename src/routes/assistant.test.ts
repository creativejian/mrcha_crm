...
test("POST /ask RAG 경로: 근거 헤더에 연락처가 병기된다", async () => {
  let userPrompt = "";
  ragFakes({ inserted: [] }, {
    getCustomerMetaByIds: async () => new Map([["c1", { name: "제임스", status: "상담중", phone: "01012345678", phoneSecondary: "0212345678" }]]),
    generateAnswer: async (_s: string, u: string) => { userPrompt = u; return "답변"; },
  });
  ...
  expect(res.status).toBe(200);
  expect(userPrompt).toContain("(제임스 · 상담중 · 연락처 01012345678 · 추가 연락처 0212345678)");
});
