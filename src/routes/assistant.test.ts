...
// 라우팅 우선(2026-07-07): 근거가 잡혀도 라우터가 먼저 판단한다. 서술형은 none → RAG(오라우팅 방지),
// 집계/목록은 call → 도구(RAG top-k로는 개수를 못 세 환각하던 견적 질문 해소). 두 골든이 양방향을 잠근다.
test("POST /ask 근거 있는 서술형은 라우터 none → RAG 경로(오라우팅 방지 골든)", async () => {
  ragFakes({ inserted: [] }, {
