...
function ragFakes(seen: RagSeen, overrides: Partial<AssistantDeps> = {}) {
  ...
  assistantDeps.getCustomerMetaByIds = async () => new Map([["c1", { name: "김민준", status: "상담중", phone: "01000000000", phoneSecondary: null }]]);
...
// 연락처 배선(2026-07-23 D1) — 실기 신고의 절반은 라우터가 도구를 안 부르고 RAG로 샌 경로였고,
// 코퍼스에 phone이 없어 **원리적으로** 답이 불가능했다. 메타 병기가 그 구조를 없앤다.
// ⚠️ 이 테스트는 route→헤더 **배선**만 잠근다. 앱 연결 고객의 합성(진짜 함정)은 실 DB 쪽
// `db/queries/embeddings-meta.test.ts`가 잡는다 — 여기 fake는 이미 합성된 값을 준다.
test("POST /ask RAG 경로: 근거 헤더에 연락처가 병기된다", async () => {
  let userPrompt = "";
  ragFakes({ inserted: [] }, {
    getCustomerMetaByIds: async () => new Map([["c1", { name: "제임스", status: "상담중", phone: "01012345678", phoneSecondary: "01098765432" }]]),
    generateAnswer: async (_s: string, u: string) => { userPrompt = u; return "답변"; },
  });

  const { token, keyResolver, issuer } = await makeTestAuth("admin");
  const app = createApp({ keyResolver, issuer });
  const res = await askJson(app, token, { question: "제임스 연락처?" });
  expect(res.status).toBe(200);
  // 표기는 화면(고객 목록·상세 formatPhone)과 같은 하이픈 포맷 — 같은 번호가 경로마다 다르게 보이면 안 된다.
  expect(userPrompt).toContain("(제임스 · 상담중 · 연락처 010-1234-5678 · 추가 연락처 010-9876-5432)");
});

// 번호가 없는 고객은 "미입력"으로 실린다 — 축을 통째로 빼면 모델이 "근거에 없음"과 구분하지 못한다.
test("POST /ask RAG 경로: 연락처 미입력도 헤더에 명시", async () => {
  let userPrompt = "";
  ragFakes({ inserted: [] }, {
    getCustomerMetaByIds: async () => new Map([["c1", { name: "무번호", status: "신규·상담접수", phone: null, phoneSecondary: null }]]),
    generateAnswer: async (_s: string, u: string) => { userPrompt = u; return "답변"; },
  });

  const { token, keyResolver, issuer } = await makeTestAuth("admin");
  const app = createApp({ keyResolver, issuer });
  await askJson(app, token, { question: "무번호 연락처?" });
  expect(userPrompt).toContain("(무번호 · 신규·상담접수 · 연락처 미입력)");
});
