...
test("상담신청 dismiss(DELETE /:id/consultations/:consultId — 배치 12 K1 이사) → consultationNote 재료 변경 → 재생성", async () => {
  ...
  try {
    ...
    // 배치 12 K1: dismiss는 customers 하위 라우트로 이사(소유권 = CUST.appUserId == 상담 userId — 위 세팅).
    const del = await app.request(`/api/customers/${CUST}/consultations/${consultationId}`, {
