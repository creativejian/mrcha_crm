...
//   **관련 질문에 딸려오는 타 고객 청크(tag-along) 0.628 ~ 0.672** ← 이 밴드가 임계값 바로 위에 있다
// 무관 최고 0.540 위에 마진을 두고 **0.60** — 관련 7/8 통과·무관 0/4 오탐(0.65면 4/8까지 떨어진다).
// 유일한 탈락(0.524 "할 일 남은 게 뭐지?")은 질의 자체가 무한정 일반적인 경우라 수용한다.
//
// ⚠️ tag-along 밴드를 **임계값으로 잘라내려 하지 마라**(배치 14 K2-a — 이 카테고리가 `#313` 재보정 때
// 주석에서 통째로 누락돼, 뒤이은 관찰자가 "0.60이 노이즈를 만들었다"고 오귀속했다). 원인은 임계값이
// 아니라 **`TOP_K = 8` 고정 + 고객당 청크 수**다: 코퍼스 161행 중 제임스 99·김민준 15·김지안 9이고
// 나머지 19명은 **정확히 2청크**라, 단일 고객 질문은 본인 청크 2개가 1·2위를 잡고 남는 6슬롯이
// 구조적으로 채워진다(실측: 본인 2위 0.6831 vs 타 고객 최상위 0.6661 — **간격 0.017**이라 어떤 절대
// 컷으로도 분리 불가). 다고객 근거는 설계 의도이기도 하다("마이바흐 관심 고객" 류 횡단 질문의 전제).
// 생성 오염은 실측으로 반증됐다 — 실 Gemini 3회 전부 대상 고객 근거만 인용했고, 오귀속 유도 질문에도
// 거부했다. 표시 노이즈를 줄이려면 **sources 표시 계층**(상대 마진·top-N)에서 다룰 것.
//
...
assistant.post("/ask", zValidator("json", askSchema), async (c) => {
  ...
  try {
    ...
    // 라우터가 none/null이면 기존 RAG 경로(hits)를 탄다. ⚠️ **"서술형이면 none"은 보장이 아니다**(배치 14
    // K2-d) — `"박서연이 원하는 조건"` 같은 서술형이 `call`로 나가는 것이 실측됐다. 아래 폴백이 그 구제책이고,
    // 1차 억제는 도구/파라미터 description의 하드닝 문구다(`assistant-tools.ts` — 그 문자열을 테스트가 잠근다).
    // 저장소의 "골든 테스트"는 `routeAssistantTool` 페이크 주입 **배선**만 잠그며 모델 판단은 원리적으로 못 잠근다.
    // none(도구 불필요)은 근거도 0건일 때만 안내 문구 — 그 경로의 한계는 아래 outOfScope 분기 주석 참조.
    ...
    if (!tool) {
      if (routed?.kind === "call") {
        ...
        // ⚠️ **정당한 0건도 함께 삼킨다**(배치 14 K2-b 실증) — 도구 결과만으로는 "오라우팅이라 0건"과
        // "제대로 골랐는데 진짜 0건"을 구분할 수 없다. 실측: `"박서연 상담신청 뭐 했어?"`는 라우터가
        // customer_consultations를 **정확히** 골랐고 도구도 **정확히** 0행을 냈는데(앱 미연결 → innerJoin
        // 구조적 0행), 이 폴백이 그 정답을 버리고 프로필의 "상담경로 앱 AI상담"을 상담신청인 것처럼
        // 서술했다. 빈도도 엣지가 아니다 — 고객 22명 중 앱 연결이 2명뿐이라 customer_consultations는
        // **20/22 고객에게 상시 0행**이다. 트레이드오프를 뒤집으려면(도구 결과+RAG 동시 적재 등)
        // pending 항목 20 회신이 필요하다 — 여기서 조건만 좁히는 수정은 `#315`가 고친 케이스를 되살린다.
        ...
        // ⚠️ 이 문구는 "잡담·일반 지식"을 전제로 만들어졌는데(assistant-prompt.ts OUT_OF_SCOPE_ANSWER),
        // `#315`가 라우터 `none`의 부류에 **"지원 필터 밖 조건의 정당한 CRM 질문"**을 더했다(배치 14 K2-c).
        // 그래서 이 분기는 SIMILARITY_THRESHOLD 위에서만 성립한다 — `"마이바흐 관심 고객이 누구야?"`는
        // 실측 top1 **0.6146**(마진 0.0146)이라 임계값이 **0.62**를 넘으면 정당한 CRM 질문이 "범위 밖"
        // 안내로 전락한다(0.65 변이로 재현 확인). 임계값을 올릴 땐 이 경로부터 볼 것.
