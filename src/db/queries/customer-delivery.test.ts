...
describe("listCustomers delivery·contractingQuote 파생 (출고 2단계 spec §4.1)", () => {
  ...
  test("contractingQuote = contracting 중 updated_at 최신 1건 + 대표 시나리오 lender (considering 제외)", async () => {
    ...
    // purchaseMethod도 대표 시나리오 경유(2026-07-24) — 계약·출고 목록의 구매방식 줄이 이 값을 쓴다.
    // 니즈(need_method)를 쓰면 "계약은 운용리스인데 목록엔 장기렌트"가 난다(실측 사례).
    const [scenario] = await db.insert(quoteScenarios).values({ quoteId: newer.id, scenarioNo: 1, purchaseMethod: "운용리스", lender: "iM캐피탈" }).returning({ id: quoteScenarios.id });
    await db.update(quotes).set({ primaryScenarioId: scenario.id }).where(eq(quotes.id, newer.id));

    const mine = (await listCustomers(db)).find((r) => r.id === cid);
    expect(mine?.contractingQuote).toEqual({ id: newer.id, brandName: "BMW", modelName: "5 Series", trimName: "520i", purchaseMethod: "운용리스", lender: "iM캐피탈" });
  });
  ...
  test("contracting 견적에 대표 시나리오가 없으면 lender만 null", async () => {
    ...
    const mine = (await listCustomers(db)).find((r) => r.id === cid);
    expect(mine?.contractingQuote).toEqual({ id: q.id, brandName: "볼보", modelName: "XC60", trimName: null, purchaseMethod: null, lender: null });
  });
