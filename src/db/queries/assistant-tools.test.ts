...
// public.consultations 픽스처만 알림 트리거를 깨운다 — 그 픽스처와 그걸 읽는 테스트 1개만 게이트하고
// 나머지 도구 테스트는 그대로 돌린다. src/test-utils/notify-gate.ts 참조.
import { notifyTriggerTest, notifyTriggerTestsEnabled } from "../../test-utils/notify-gate";
...
beforeAll(async () => {
  ...
  // public.consultations INSERT는 on_consultation_created 트리거 → 운영 디스코드 알림을 낸다(게이트).
...
afterAll(async () => {
  if (notifyTriggerTestsEnabled) { // 게이트 off면 상담신청 픽스처가 없다(id 빈 문자열 — DELETE 자체를 보내지 않는다)
...
// 이 테스트만 상담신청 픽스처(=알림 트리거)를 필요로 한다 — 게이트 off면 skip.
notifyTriggerTest("customer_consultations: 연결된 고객의 상담신청이 관심 차종·문의 내용과 함께 반환되고, 숨김 처리된 건은 제외된다", async () => {
