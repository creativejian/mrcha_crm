...
// updateQuote(appStatus:"sent") 발송 훅이 ex를 그대로 upsertAdvisorQuote에 넘긴다 → tx를 주면
// public.advisor_quotes upsert까지 같은 트랜잭션에 들어와 on_advisor_quote_sent 가드가 걸린다.
...
test("발송: advisor_quotes 행 생성 — 정규 컬럼·스탬프 정합 + payload 라벨 완성본", async () => {
  ...
  try {
    ...
    const res = await withNotifyGuard(db, (tx) => updateQuote(ids.customerId!, created.id, { appStatus: "sent" }, tx));
...
test("재발송(워크벤치 결합 patch): 행 1개 유지·viewed_at 리셋·시나리오 교체 후 값 스냅샷·revision 반영", async () => {
  ...
  try {
    ...
    await withNotifyGuard(db, (tx) => updateQuote(ids.customerId!, created.id, { appStatus: "sent" }, tx));
    // 앱 사용자가 열람한 상태 재현 — 재발송이 이걸 NULL로 리셋해야 한다.
    // viewed_at만 바꾸는 UPDATE는 현재 notify_advisor_quote 트리거의 2분기(sent_at 미변경 → early return)
    // 덕에 알림이 안 나가지만, 그건 **앱 소유 트리거의 구현**에 안전을 위탁하는 것이다(앱이 열람 알림을
    // 추가하면 테스트 1회가 실 고객 FCM을 쏜다 — #199 재현). 방어는 guard가 한다.
    await withNotifyGuard(db, (tx) =>
      tx
    ...
    await withNotifyGuard(db, (tx) => updateQuote(
      ids.customerId!,
      created.id,
      {
        appStatus: "sent",
...
test("마지막 견적 회수: 잔여 카드 0이면 quote_requests completed→open 복원(앱 '완료인데 견적 없음' 모순 방지)", async () => {
  const userId = await anyProfileId();
  const ids: { quoteId?: string; customerId?: string; requestId?: string } = {};
  try {
    ids.requestId = crypto.randomUUID();
    ...
    await withNotifyGuard(db, (tx) => updateQuote(ids.customerId!, created.id, { appStatus: "sent" }, tx));
    const [sent] = await db.select({ status: quoteRequests.status }).from(quoteRequests).where(eq(quoteRequests.id, ids.requestId));
    expect(sent.status).toBe("completed");

    await deleteQuote(ids.customerId, created.id, db);
    const [after] = await db.select({ status: quoteRequests.status }).from(quoteRequests).where(eq(quoteRequests.id, ids.requestId));
    expect(after.status).toBe("open"); // 마지막 카드 회수 → 요청은 다시 진행중(앱 정책 제안 2026-07-05)
  } finally {
    await cleanup(ids);
  }
});

test("일부 회수: 같은 요청에 다른 카드가 남아 있으면 completed 유지", async () => {
  const userId = await anyProfileId();
  const ids: { quoteId?: string; customerId?: string; requestId?: string } = {};
  ...
  try {
    ...
    await db.insert(quoteRequests).values({ id: ids.requestId, userId, trimId: null, status: "open", createdAt: new Date().toISOString() });
    ids.customerId = await makeCustomer(userId);
    const first = await createQuote(ids.customerId, baseQuoteBody({ sourceQuoteRequestId: ids.requestId }), db);
    ids.quoteId = first.id;
    const second = await createQuote(ids.customerId, baseQuoteBody({ sourceQuoteRequestId: ids.requestId }), db);
    secondQuoteId = second.id;
    await withNotifyGuard(db, (tx) => updateQuote(ids.customerId!, first.id, { appStatus: "sent" }, tx));
    await withNotifyGuard(db, (tx) => updateQuote(ids.customerId!, second.id, { appStatus: "sent" }, tx));

    await deleteQuote(ids.customerId, first.id, db);
    const [after] = await db.select({ status: quoteRequests.status }).from(quoteRequests).where(eq(quoteRequests.id, ids.requestId));
    expect(after.status).toBe("completed"); // 고객 견적함에 카드가 아직 있으므로 완료 유지
  } finally {
    if (secondQuoteId) await deleteQuote(ids.customerId!, secondQuoteId, db); // advisor 카드까지 회수 경로로 정리
    await cleanup(ids);
  }
});

test("closed 요청은 회수로 되살리지 않는다(어드민 수동 마감 불가침)", async () => {
  const userId = await anyProfileId();
  const ids: { quoteId?: string; customerId?: string; requestId?: string } = {};
  try {
    ids.requestId = crypto.randomUUID();
    ...
    ids.quoteId = created.id;
    await withNotifyGuard(db, (tx) => updateQuote(ids.customerId!, created.id, { appStatus: "sent" }, tx));
    // 발송 후 어드민이 요청을 수동 마감한 상황
    await db.update(quoteRequests).set({ status: "closed" }).where(eq(quoteRequests.id, ids.requestId));

    await deleteQuote(ids.customerId, created.id, db);
    const [after] = await db.select({ status: quoteRequests.status }).from(quoteRequests).where(eq(quoteRequests.id, ids.requestId));
    expect(after.status).toBe("closed"); // completed일 때만 open 복원 — closed는 불가침
  } finally {
    await cleanup(ids);
  }
});
...
test("read-through(Task 5): 앱 열람 시각이 getCustomer quotes[].viewedAt에 병합된다", async () => {
  ...
  try {
    ...
    await withNotifyGuard(db, (tx) => tx.update(advisorQuotes).set({ viewedAt: viewed.toISOString() }).where(eq(advisorQuotes.crmQuoteId, created.id)));
