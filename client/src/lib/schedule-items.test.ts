...
import {
  scheduleSortValue,
  scheduleRecordKey,
  sortCheckItemsByWorkRule,
  sortCustomerMemosByCreatedAt,
  sortSchedulesByDateTime,
  type CheckItem,
  type CustomerMemoItem,
  type ScheduleItem,
} from "./schedule-items";
...
describe("sortCustomerMemosByCreatedAt", () => {
  ...
  it("같은 시각이면 id로 안정 정렬한다", () => {
    ...
    expect(sortCustomerMemosByCreatedAt(memos).map((m) => m.id)).toEqual(["a", "z"]);
  });
  it("원본 배열을 변형하지 않는다(불변)", () => {
    ...
    sortCustomerMemosByCreatedAt(memos);
    expect(memos.map((m) => m.id)).toEqual(["a", "b"]);
  });
});

describe("sortCheckItemsByWorkRule", () => {
  ...
  it("미완료는 급함→오늘→내일→이번 주 순", () => {
    expect(sortCheckItemsByWorkRule(items, []).map((i) => i.id)).toEqual([
      "urgent",
      "today",
      "tomorrow",
      "week",
  ...
  it("완료 항목을 미완료보다 위로 올린다", () => {
    expect(sortCheckItemsByWorkRule(items, ["week"]).map((i) => i.id)).toEqual([
      "week",
      "urgent",
      "today",
      "tomorrow",
  ...
  it("지정 날짜(M/D)는 같은 due 랭크 내에서 날짜 오름차순", () => {
    const dated: CheckItem[] = [
      { id: "late", category: "", due: "5/20", body: "" },
      { id: "early", category: "", due: "5/10", body: "" },
    ];
    expect(sortCheckItemsByWorkRule(dated, []).map((i) => i.id)).toEqual(["early", "late"]);
  });
});

describe("scheduleSortValue / sortSchedulesByDateTime", () => {
  it("빈 날짜/시간은 맨 뒤로 가는 fallback 값", () => {
    expect(scheduleSortValue({ id: "x", date: "", time: "", type: "", memo: "" })).toBe("9999-12-31T23:59");
    expect(scheduleSortValue({ id: "y", date: "2026-05-14", time: "09:00", type: "", memo: "" })).toBe("2026-05-14T09:00");
  });
  it("날짜+시간 오름차순으로 정렬한다", () => {
    ...
    expect(sortSchedulesByDateTime(schedules).map((s) => s.id)).toEqual(["a", "b", "c"]);
  });
});
