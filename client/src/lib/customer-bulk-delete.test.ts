...
import { deleteCustomersBulk, formatBulkTargetNames } from "./customer-bulk-delete";
...
describe("formatBulkTargetNames", () => {
  it("1명은 이름 그대로", () => {
    expect(formatBulkTargetNames(["제임스"])).toBe("제임스");
  });

  it("5명까지는 전부 나열", () => {
    expect(formatBulkTargetNames(["가", "나", "다", "라", "마"])).toBe("가, 나, 다, 라, 마");
  });

  it("6명부터는 앞 5명 + 외 N명", () => {
    expect(formatBulkTargetNames(["가", "나", "다", "라", "마", "바"])).toBe("가, 나, 다, 라, 마 외 1명");
    expect(formatBulkTargetNames(["가", "나", "다", "라", "마", "바", "사"])).toBe("가, 나, 다, 라, 마 외 2명");
  });

  it("빈 이름은 '이름 없음'으로 대체한다(빈 문자열이 구분자만 남기지 않게)", () => {
    expect(formatBulkTargetNames(["제임스", "  "])).toBe("제임스, 이름 없음");
  });

  it("선택 0명이면 빈 문자열", () => {
    expect(formatBulkTargetNames([])).toBe("");
  });
});
