...
export function AdminDashboardPage() {
  ...
  // 0건은 고장이 아니라 사실이다 — 다만 원인이 "계약 확정일 미입력"인 경우가 대부분이라 안내한다.
  // ⚠️ 귀속 기준은 차량 인도일이 아니라 **계약 확정일**이다(2026-08-03 이사님) — 현장에서 그 확정을
  //    "출고"라고 부르기 때문에 칩 라벨은 "전체 출고" 그대로 두되 기준은 확정일이다.
  const heroHint =
    report && report.delivery.count === 0
      ? "이 달 확정된 계약이 없습니다 — 출고 정보에 계약 확정일을 입력하면 집계됩니다."
      : "계약 확정일 기준입니다. 리스는 취득원가, 렌트는 차량가로 집계합니다.";
