...
function buildCardDom(condId: string, values: Record<string, string> = {}) {
  ...
  card.append(lender);
  // 판매사 select(T2) — 프로덕션 option은 dealerOptionsByCard(비동기 fetch) 산물. 테스트는 값 대입이
  // 가능하도록 "" + 전달된 값을 정적 option으로 깐다(uncontrolled select 값 대입은 option 기존재가 전제).
...
describe("useQuoteWorkbench — 솔루션 조회 결과 반영·늦은 응답 잔상 가드", () => {
  it("조회 성공: 월납입(표시 라운딩)·잔가 채움 + 결과 4필드는 리스계산기 파생 + 스냅샷이 저장 payload에 동봉된다(가드 대조군)", async () => {
    ...
    const card = buildCardDom("manual-condition-1", { lender: "iM캐피탈", cmFeePercent: "1.5", agFeePercent: "2" });
    ...
    expect(scenarios[0]).toMatchObject({
      ...
      interestRate: "23.16",
      // CM/AG %(#264) — 추출(저장 스냅샷)이 카드 DOM 입력을 payload에 싣는지 잠금(상이 값 = 교차 배선 검출).
      cmFeePercent: "1.5",
      agFeePercent: "2",
      solutionLenderCode: "im-capital",
      solutionWorkbookVersion: "2026-07 v2",
  ...
  it("2-A 저장 카드 게이트: 레거시 견적(수기 금리) 재진입은 결과 4필드를 파생값으로 덮지 않는다", async () => {
    ...
    await act(async () => {
      result.current.openEditQuote({ id: "q-edit-legacy" } as unknown as QuoteItem);
    });
  ...
  it("늦은 응답 가드: 조회 중 카드가 언마운트(견적 전환)되면 stale 스냅샷을 병합하지 않는다(#163 잔상 부류)", async () => {
    ...
    requestSolution.mockImplementation(
      () =>
        new Promise((r) => {
          resolveLate = r;
        }),
    ...
    act(() => {
      pending = result.current.handlers.queryCardSolution("manual-condition-1");
    });
...
// ── 판매사(딜러) T2 — 모드 게이트·전사 프로브 strip·금융사 변경 리셋 ─────────────────────────────
// 값의 진실 원본 = 카드 controlled state(manualQuoteCards[].dealerName) — 금융사 select 계약 미러(②c).
// buildCardDom의 딜러 select는 detach 가드(cardEl.isConnected)·조회 payload(buildCardSolutionArgs가 DOM에서
// 읽음)용 DOM일 뿐이라, 저장·리셋 검증은 state(setManualDealer/manualQuoteCards)로 시드·대조한다.

describe("useQuoteWorkbench — 판매사(딜러) 조회 payload·영속·리셋", () => {
  it("판매사 입력 모드: 직접 계산 payload에 dealerName 동봉(해당 금융사 단일 요청) + 저장 시나리오에 plain dealer_name 영속", async () => {
    const { result } = setup();
    const { pricingRoot, compareForm } = await setupSolutionDom(result);
    const card = buildCardDom("manual-condition-1", { lender: "iM캐피탈", dealer: "도이치모터스" });
    compareForm.append(card);
    // 금융사·판매사는 controlled state(②c) — extract가 state에서 읽으므로 시드한다. lender 먼저(dealer 청소)→dealer.
    act(() => {
      result.current.handlers.setManualLender("manual-condition-1", "iM캐피탈");
      result.current.handlers.setManualDealer("manual-condition-1", "도이치모터스");
    });
    ...
    // 직접 계산은 금융사가 동일 값(멱등)이라 딜러 리셋이 발동하면 안 된다 — 리셋되면 아래 payload·저장이 비어 실패.
    expect(requestSolution).toHaveBeenCalledWith(expect.objectContaining({ dealerName: "도이치모터스" }));
    const scenarios = await savedScenarios(result);
    expect(scenarios).toHaveLength(1);
    expect(scenarios[0].dealerName).toBe("도이치모터스"); // plain 이름(계산기 `lenderCode::dealerName` 합성 아님)
    pricingRoot.remove();
    compareForm.remove();
  });

  it("비제휴 계산(기본 모드): DOM에 딜러 값이 남아 있어도 조회 payload 미전송·저장 null(모드 게이트)", async () => {
    const { result } = setup();
    const { pricingRoot, compareForm } = await setupSolutionDom(result);
    const card = buildCardDom("manual-condition-1", { lender: "iM캐피탈", dealer: "도이치모터스" });
    compareForm.append(card);
    // 금융사는 controlled state(②c) — isFilled 판정이 state lender를 읽으므로 시드(딜러는 모드 게이트로 어차피 null).
    act(() => result.current.handlers.setManualLender("manual-condition-1", "iM캐피탈"));
    requestSolution.mockClear();
    requestSolution.mockResolvedValue(partnerResponse);
    await act(async () => {
      await result.current.handlers.queryCardSolution("manual-condition-1");
  ...
  it("랭킹 모달 조립(buildCardSolutionBaseArgs)은 딜러를 항상 벗긴다 — 전사 병렬 프로브의 타사 무음 오염 구조 차단", async () => {
    const { result } = setup();
    const { pricingRoot, compareForm } = await setupSolutionDom(result);
    const card = buildCardDom("manual-condition-1", { lender: "iM캐피탈", dealer: "도이치모터스" });
    compareForm.append(card);
    act(() => result.current.handlers.setManualDealerMode("manual-condition-1", "input"));
    // 같은 카드·같은 조건 — 직접 계산 조립엔 실리는 값(위 테스트)이 프로브 조립에선 null이어야 한다.
    expect(result.current.handlers.buildCardSolutionBaseArgs("manual-condition-1")?.dealerName).toBeNull();
    pricingRoot.remove();
    compareForm.remove();
  });

  it("금융사 변경(setManualLender 커밋 핸들러): 딜러 선택 리셋(타사 딜러 잔존 = 무음 오계산) + (금융사,브랜드) 목록 적재", async () => {
    const { result } = setup();
    const { pricingRoot, compareForm } = await setupSolutionDom(result); // 브랜드 = BMW
    const card = buildCardDom("manual-condition-1"); // 카드 DOM은 detach 가드(cardEl.isConnected)용 — 값은 state가 진실(②c)
    compareForm.append(card);
    act(() => result.current.handlers.setManualDealer("manual-condition-1", "도이치모터스")); // 잔존 딜러 재현(state)
    fetchDealers.mockClear();
    // 딜러 fetch는 지연 resolve — 늦은 응답 가드는 이제 state(ref)를 읽으므로, mock이 setState 커밋보다
    // 먼저 resolve하면 stale(미선택)을 봐 응답을 버린다(prod에선 네트워크가 커밋보다 느려 무관). 커밋 후 resolve.
    let resolveDealers!: (v: SolutionDealer[]) => void;
    fetchDealers.mockImplementation(
      () =>
        new Promise((r) => {
          resolveDealers = r;
        }),
    ...
    await act(async () => {
      result.current.handlers.setManualLender("manual-condition-1", "iM캐피탈"); // controlled 커밋 = 델리게이션 대체
      await Promise.resolve(); // setState 커밋(ref=iM) 후에야 아래 fetch를 resolve
    });
    // 금융사가 바뀌었으므로 딜러는 청소된다(딜러 = 금융사 귀속) — state로 확인.
    expect(result.current.manualQuoteCards.find((c) => c.id === "manual-condition-1")!.dealerName).toBe("");
    await act(async () => {
      resolveDealers([{ dealerName: "바바리안모터스", baseIrrRate: 0.0681 }]);
      await Promise.resolve();
    });
    expect(fetchDealers).toHaveBeenCalledWith("im-capital", "BMW");
    expect(result.current.dealerOptionsByCard["manual-condition-1"]).toEqual([{ dealerName: "바바리안모터스", baseIrrRate: 0.0681 }]);
    pricingRoot.remove();
    compareForm.remove();
  });

  // 구매방식 전환 D4 정리(②c 신 정책): controlled에서는 금융사가 state에 남아, 전환으로 그 금융사가 화면
  // 가능 집합(현행 어휘·자기 seed)에서 빠지면 state를 직접 "미선택"으로 되돌린다 + 토스트 1회(구 조용한
  // 유실 → 명시 안내로 승격). 딜러 목록도 이전 금융사 기준이라 stale — 함께 정리된다(setManualLender→
  // loadCardDealers(미선택)가 키 삭제). 구 uncontrolled의 "이벤트 없는 브라우저 리셋" 전제는 사라졌다.
  it("구매방식 전환(D4): 미취급 금융사를 미선택으로 되돌리고 딜러 목록·토스트까지 정리한다(배치 13 별건 ②c)", async () => {
    const { result, onToast } = setup();
    const { pricingRoot, compareForm } = await setupSolutionDom(result); // 브랜드 = BMW, 구매방식 운용리스
    const card = buildCardDom("manual-condition-1");
    compareForm.append(card);
    fetchDealers.mockClear();
    let resolveDealers!: (v: SolutionDealer[]) => void;
    ...
    // arrange — 금융사를 골라 딜러 목록을 적재한다(controlled 커밋 경로). 커밋 후 fetch를 resolve(늦은 응답 가드 계약).
    await act(async () => {
      result.current.handlers.setManualLender("manual-condition-1", "iM캐피탈");
      await Promise.resolve();
    });
    ...
    expect(result.current.manualQuoteCards.find((c) => c.id === "manual-condition-1")!.lender).toBe("iM캐피탈");

    // act — 할부로 전환: 파트너 미구현(어휘 0)이라 iM캐피탈이 화면 가능 집합에서 빠진다.
    onToast.mockClear();
    await act(async () => {
    ...
    // assert — 금융사는 미선택으로 리셋(state) + 딜러 목록 stale 제거 + 명시 토스트.
    expect(result.current.manualQuoteCards.find((c) => c.id === "manual-condition-1")!.lender).toBe("미선택");
    expect(result.current.dealerOptionsByCard["manual-condition-1"]).toBeUndefined();
    expect(onToast).toHaveBeenCalledWith(expect.stringContaining("미취급"));
    pricingRoot.remove();
    compareForm.remove();
  });
});
...
describe("useQuoteWorkbench — 승격 컬러 프리필", () => {
  it("selected 컬러 id로 외장·내장을 프리필한다", async () => {
    ...
    await act(async () => result.current.openWorkbenchForQuoteRequest("req-1"));
    await act(async () => {
      await result.current.handlers.applyTrimToPricing(vehicleSelectionWithColors);
    });
...
// 금융사·판매사는 controlled state(②c)라 state로 시드·대조하고(stateLender/manualQuoteCards), 금액 input은
// 여전히 uncontrolled DOM이라 카드 DOM을 직접 대조한다(복사도 그 두 층을 각각의 원본에서 옮긴다).
...
function setupCopyDom(result: ReturnType<typeof setup>["result"], sourceValues: Record<string, string> = {}) {
  ...
  // 금융사·판매사는 controlled state가 진실(②c) — sourceValues의 그 두 축을 카드1 state에도 시드한다
  // (DOM만으론 copy가 못 읽는다). 반드시 lender 먼저(setManualLender가 dealerName을 청소하므로) → dealer.
  act(() => {
    if (sourceValues.lender) result.current.handlers.setManualLender("manual-condition-1", sourceValues.lender);
    if (sourceValues.dealer) result.current.handlers.setManualDealer("manual-condition-1", sourceValues.dealer);
  });
  return { card1, card2, card3 };
}

// 카드의 controlled 금융사 state(②c)를 읽는다 — DOM select가 아니라 진실을 본다.
const stateLender = (result: ReturnType<typeof setup>["result"], condId: string) =>
  result.current.manualQuoteCards.find((c) => c.id === condId)!.lender;
...
describe("useQuoteWorkbench — N번 조건 복사(비교카드 헤더)", () => {
  it("1번 복사: 금융사·조건 입력값(DOM)과 카드 UI 상태(기간/모드/약정거리/자동차세/보조금)를 카드2로 복사한다", () => {
    ...
    act(() => {
      result.current.handlers.copyManualQuoteCondition("manual-condition-2", "2");
    });
    expect(stateLender(result, "manual-condition-2")).toBe("iM캐피탈");
    ...
    // CM/AG %(#264) — 복사 루프(if (src && dst) null-가드)가 필드명 이탈 시 무음 no-op이라 여기서 잠근다.
    // 상이 값(1.5 ≠ 2)이라 cm↔ag 교차 배선도 정확히 실패한다.
    expect(domField(card2, "cmFeePercent")).toBe("1.5");
    expect(domField(card2, "agFeePercent")).toBe("2");
    // 판매사(T2): controlled state 복사(②c) — 금융사와 같은 축(dealerMode 복사는 아래 cardUi 통째 비교가 커버).
    expect(result.current.manualQuoteCards.find((c) => c.id === "manual-condition-2")!.dealerName).toBe("도이치모터스");
  ...
  it("월납입·결과 4필드는 조회 파생값이라 복사하지 않는다", () => {
    ...
    const { card2 } = setupCopyDom(result, {
      ...
      monthly: "1,234,600",
      interestRate: "5.32",
      totalReturn: "80,000,000",
      totalTakeover: "95,000,000",
      dueAtDelivery: "5,000,000",
    });
    act(() => {
      result.current.handlers.copyManualQuoteCondition("manual-condition-2", "2");
    });
    expect(stateLender(result, "manual-condition-2")).toBe("iM캐피탈"); // 복사 자체는 수행됐다(대조군)
  ...
  it("2번 복사: 카드3은 카드2(직전 카드)에서 복사한다 — 카드1이 아니라", () => {
    ...
    act(() => result.current.handlers.setManualLender("manual-condition-2", "iM캐피탈")); // 금융사 = state(②c)
    card2.querySelector<HTMLInputElement>('input[data-sc-field="deposit"]')!.value = "7,000,000"; // 보증금 = uncontrolled DOM
    act(() => {
      result.current.handlers.setManualTermMonthsFor("manual-condition-2", 24);
    });
    act(() => {
      result.current.handlers.copyManualQuoteCondition("manual-condition-3", "3");
    });
    expect(stateLender(result, "manual-condition-3")).toBe("iM캐피탈");
  ...
  it("저장(잠금)된 대상 카드에는 복사하지 않는다 — 버튼 disabled 미러 가드", () => {
    ...
    act(() => {
      result.current.handlers.saveManualQuoteCondition("manual-condition-2", "2");
    });
    act(() => {
      result.current.handlers.copyManualQuoteCondition("manual-condition-2", "2");
    });
    expect(stateLender(result, "manual-condition-2")).toBe("미선택");
    expect(domField(card2, "deposit")).toBe("0");
    expect(result.current.cardUi["manual-condition-2"]).toBeUndefined();
  });

  it("구 어휘 금융사(대상 카드에서 렌더 불가)는 무음 부분 복사 대신 토스트로 제외를 알린다 — 그 금융사 귀속인 딜러도 함께 제외", () => {
    const hook = setup();
    const { result, onToast } = hook;
    // 소스 카드1: 레거시 금융사 "구캐피탈"(현행 어휘·대상 seed 어디에도 없음) + 딜러 "모터원"을 state에 시드.
    // setupCopyDom이 lender 먼저(dealer 청소) → dealer 순서로 심어 둘이 공존한다(②c seed 계약).
    const { card2 } = setupCopyDom(result, { deposit: "5,000,000", lender: "구캐피탈", dealer: "모터원" });
    act(() => {
      result.current.handlers.setManualDealerMode("manual-condition-1", "input");
    });
    act(() => {
    ...
    expect(stateLender(result, "manual-condition-2")).toBe("미선택"); // 대상이 렌더 못 하는 값 — 복사 제외
