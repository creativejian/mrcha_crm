...
export function kimQuoteAppStatusLabel(status: QuoteItem["appStatus"], quote?: QuoteItem) {
...
export function kimQuoteAppSendLabel(status: QuoteItem["appStatus"], quote?: QuoteItem) {
  if ((quote?.revision ?? 1) > 1 && status === "viewed") return "수정 열람";
  if ((quote?.revision ?? 1) > 1 && status === "sent") return "수정 발송";
  if (status === "viewed") return "고객 열람";
  if (status === "sent") return "발송 완료";
...
function kimQuoteRevisionLabel(quote: QuoteItem) {
  if (!quote.revision || quote.revision <= 1) return null;
  return `수정 v${quote.revision}`;
}

export function kimQuoteStatusDetailParts(quote: QuoteItem) {
...
export function kimQuoteDeleteConfirmTitle(quote: QuoteItem) {
...
export function kimQuoteDeleteConfirmMessage(quote: QuoteItem) {
  if (quote.appStatus === "sent" || quote.appStatus === "viewed") {
...
export function kimQuoteDecisionLabel(status: QuoteItem["decisionStatus"]) {
...
export function kimQuoteStockClass(status?: QuoteItem["stockStatus"]) {
