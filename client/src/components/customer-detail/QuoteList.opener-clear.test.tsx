...
function Harness({ writable = true }: { writable?: boolean } = {}) {
  ...
  return (
    <QuoteList
      ...
      quoteWritable={writable}
    />
  );
}

function mount(quote: QuoteItem, writable = true) {
  render(<Harness writable={writable} />);
...
describe("견적 행 액션 오프너 — 팝오버-내부 상태 동반 클리어 전수 (배치 12 B#1 · 배치 13 K2-a)", () => {
  ...
  it("표준 상태(draft·쓰기 가능)의 팝오버 버튼 집합 = OPENERS(신규 오프너가 목록에 등재되지 않으면 RED)", async () => {
    const user = userEvent.setup();
    mount(baseQuote);
    await user.click(screen.getByRole("button", { name: "테스트 견적 견적 작업 열기" }));

    const rendered = within(popover())
      .getAllByRole("button")
      .map((b) => b.textContent?.trim() ?? "");
    expect([...rendered].sort()).toEqual([...OPENERS].sort());
  });

  // 위 케이스는 **표준 상태 한 프레임만** 본다 — 다른 상태에서만 나타나는 버튼은 등재 없이 통과한다
  // (배치 15 M9). 실제로 `appStatus:"sent"`에서는 OPENERS에 없는 **"발송 견적 보기"**가 렌더되는데,
  // draft만 보는 위 케이스는 그걸 영원히 못 본다. 그래서 상태 축(쓰기 권한 × 발송 여부) 4칸의 버튼 집합을
  // 각각 못박는다. 기대값은 추측이 아니라 **실제 렌더 결과를 프로브로 확정한 것**이다.
  // ※ "발송 견적 보기"는 클릭 시 팝오버를 닫으므로(실측: 클릭 후 열린 dialog 0개) **오프너가 아니다**
  //   — 그래서 OPENERS(넛지·downgrade 클리어 대상)에는 넣지 않는다. 원본 보기/첨부와 같은 계열이다.
  const BUTTON_SETS: ReadonlyArray<readonly [string, QuoteItem, boolean, readonly string[]]> = [
    ["draft · 쓰기 가능", baseQuote, true, OPENERS],
    ["draft · 읽기 전용", baseQuote, false, []],
    [
      "sent · 쓰기 가능",
      { ...baseQuote, appStatus: "sent" },
      true,
      ["발송 견적 보기", "견적 수정", "최종 고민중", "고객 확정", "계약 진행", "삭제"],
    ],
    ["sent · 읽기 전용", { ...baseQuote, appStatus: "sent" }, false, ["발송 견적 보기"]],
  ];

  it.each(BUTTON_SETS)("팝오버 버튼 집합 잠금 — %s", async (_name, quote, writable, expected) => {
    const user = userEvent.setup();
    mount(quote, writable);
    await user.click(screen.getByRole("button", { name: "테스트 견적 견적 작업 열기" }));
  ...
  it("'견적 수정' 오프너는 열려 있던 넛지를 지운다(K2-a 사용자 증상)", async () => {
    const user = userEvent.setup();
    mount(baseQuote);
    await user.click(screen.getByRole("button", { name: "테스트 견적 견적 작업 열기" }));
    await user.click(openerButton("계약 진행"));
    await user.click(within(popover()).getByRole("button", { name: "확정" }));
    expect(screen.getByRole("dialog", { name: "진행 상태 계약완료 변경 확인" })).toBeTruthy();
  ...
  // 2026-07-24: 계약 진행 견적도 수정할 수 있다(구 동작은 실체 없는 "계약 관리 창"으로 안내만 하고
  // 막았다 — spec 2026-07-20-crm-delivery-step2-design §113). 다만 고객 앱에 "진행 중인 계약"으로
  // 보이는 견적이라 경고를 한 번 거친다. 경고 판단은 **오프너**가 한다 — openEditQuote에 두면
  // 다이얼로그를 못 그리는 경로(니즈 카드)가 무음으로 잠긴다(K2-c).
  it("계약 진행 견적의 '견적 수정'은 경고를 먼저 띄우고, 확인하면 그대로 연다", async () => {
    const user = userEvent.setup();
    mount({ ...baseQuote, decisionStatus: "contracting" });
    await user.click(screen.getByRole("button", { name: "테스트 견적 견적 작업 열기" }));
    ...
    expect(screen.getByRole("dialog", { name: "계약 진행 견적 수정 확인" })).toBeTruthy();
    expect(onEditQuote).not.toHaveBeenCalled(); // 경고 없이 바로 열리면 안 된다
