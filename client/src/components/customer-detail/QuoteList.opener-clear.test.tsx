...
describe("견적 행 액션 오프너 — 팝오버-내부 상태 동반 클리어 전수 (배치 12 B#1 · 배치 13 K2-a)", () => {
  ...
  const BUTTON_SETS: ReadonlyArray<readonly [string, QuoteItem, boolean, readonly string[]]> = [
    ...
    ["sent · 쓰기 가능", { ...baseQuote, appStatus: "sent" }, true, ["발송 견적 보기", "견적 수정", "최종 고민중", "고객 확정", "계약 진행", "삭제"]],
  ...
  it.each(BUTTON_SETS)("팝오버 버튼 집합 잠금 — %s", async (_name, quote, writable, expected) => {
    ...
    const rendered = within(popover()).queryAllByRole("button").map((b) => b.textContent?.trim() ?? "");
