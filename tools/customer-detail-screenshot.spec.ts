...
test("opens customer detail from the all-customer list", async ({ page }) => {
  ...
  const manualQuoteDialog = page.getByRole("dialog", { name: "견적 작성" });
  await expect(manualQuoteDialog).toBeVisible();
  await expect(manualQuoteDialog.getByRole("button", { name: "운용리스", exact: true })).toHaveAttribute("aria-pressed", "true");
  await expect(manualQuoteDialog.getByRole("button", { name: "솔루션 조회" }).first()).toHaveAttribute("aria-pressed", "true");
  await expect(manualQuoteDialog.getByRole("textbox", { name: "월 납입금" })).toHaveValue("월 2,473,200원");
  await manualQuoteDialog.getByRole("button", { name: "장기렌트", exact: true }).click();
  await expect(manualQuoteDialog.getByRole("button", { name: "장기렌트", exact: true })).toHaveAttribute("aria-pressed", "true");
  await expect(manualQuoteDialog.getByRole("textbox", { name: "월 렌트료" })).toHaveValue("월 1,642,190원");
  await expect(manualQuoteDialog.getByRole("textbox", { name: "보험 포함" })).toBeVisible();
  await page.getByRole("button", { name: "견적 작성 닫기" }).click();
