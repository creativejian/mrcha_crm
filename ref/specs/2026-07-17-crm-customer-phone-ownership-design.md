...
- ~~**인박스 "고객 생성" 중복 고객**~~ → **✅ 해소(예방, #282 `04ee963`, 2026-07-18)**: `none` 매칭일 때 같은 이름 미연결 고객을 제안 노출 → 기존 link 재사용으로 중복 예방(병합 엔진 미도입). 두 인박스 미러(spec `2026-07-18-crm-inbox-name-match-suggestion-design.md`). 사후 병합(레코드 결합)은 여전히 후속.
