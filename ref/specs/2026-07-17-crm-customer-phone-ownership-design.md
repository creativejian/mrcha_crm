...
4. **연결(link) 시 전이 규칙**: 대상 고객의 기존 `phone`이
   ...
   - **각주(2026-07-18 배치 8 C#2, 유슨생 결정)**: secondary가 **앱 번호와 같은 값**으로 차 있으면 점유가 아니라 **중복 정보**로 본다 — 연결 후 주 번호가 profiles 파생으로 같은 값을 이미 표시하므로, 그 자리에 기존 phone(실번호)을 교체·보존하고 `droppedPhone`은 반환하지 않는다(구 동작은 점유 판정 → 동일 번호 2회 병기 + 실번호 폐기). 위 "값이 다르면 점유" 문언은 이 중복 케이스를 제외한 나머지에만 적용된다.
...
- ~~**인박스 "고객 생성" 중복 고객**~~ → **✅ 해소(예방, #282 `04ee963`, 2026-07-18)**: `none` 매칭일 때 같은 이름 미연결 고객을 제안 노출 → 기존 link 재사용으로 중복 예방(병합 엔진 미도입). 두 인박스 미러(spec `2026-07-18-crm-inbox-name-match-suggestion-design.md`). **사후 병합(이미 생긴 중복 레코드 결합)은 보류 — 2026-07-18 실측 시 중복 0건**(전체 22명 중 같은 이름 2명↑ 그룹 0·연결/미연결 혼재 0). #282가 신규 중복도 막으므로 실착수 압력 없음. 재론 트리거 = "같은 이름 + 연결/미연결 혼재" 그룹이 실제 관측될 때(그때 자식 재부모·app_user_id unique·phone CHECK·충돌 필드 승자·확인 UI를 설계).
- **연결 시 droppedPhone 선택 UI**(v1은 토스트 알림만) — **현행 토스트 유지 결정(유슨생 2026-07-18)**: 배치 8 C#2 이후 발생 조건이 세 번호 전부 다른 드문 경우로 좁아졌고 무음도 아님. 재론 트리거 = 실사용 요구.
- ~~**Topbar 전역 통합검색이 mock 검색**~~ → **✅ 해소(#281 `2c704a6`, 2026-07-18)**: 실 `customers` prop 배선.
