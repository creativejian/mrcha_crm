...
## 개정 3 — 확장 필드 17종 전송·제프 회신 박제 (2026-07-16, 계산기 모달 #262)

- **CRM이 external calculate에 확장 필드 17종 전송 개시**(#262 전역 계산기 모달):
  취득원가 계열(acquisitionTaxMode·acquisitionTaxAmountOverride·includePublicBondCost/publicBondCost·
  includeDeliveryFeeAmount/deliveryFeeAmount·includeMiscFeeAmount/miscFeeAmount) ·
  수수료(cmFeeRate·agFeeRate) · 렌트(releaseMethod·maintenanceGrade) ·
  기타(affiliateType='비제휴사' 고정·directModelEntry·selectedResidualRateOverride·
  insuranceYearlyAmount·lossDamageAmount).
- **제프 확인(2026-07-16 회신)**: external은 내부와 **동일 `calculateQuoteSchema` 검증** — 전 필드
  정상 소비. "이 필드 수용 전제 유지"가 **제프 내부 리팩토링 체크리스트에 등재**(계약 안전망 성립).
- **⚠️ strip 양날**: 스키마는 미지 필드를 400이 아니라 **조용히 무시(strip)** — forward-compat은
  보장되지만 오타·미정의 신필드도 조용히 사라진다. **새 필드 추가 시 200 OK가 아니라 계산 결과
  변화로 소비를 검증할 것**(CRM 릴레이 zod strip과 동일 메커니즘 — 양쪽 대칭 주의).
- **레이트리밋**: 현재 없음(X-API-Key 검증만) — 시나리오 3×전사 병렬 허용. 향후 트래픽 따라
  제프가 키 정책 단위 도입 예정, 도입 시 CRM 통지 전제.
- **`/api/external/quotes/cheapest` 미채택**: 계산기·랭킹 모달 요건 = 전사 결과 나열(정렬·배지·
  차액·선택 최대 3)이라 최저가 단건 반환과 불일치. 선례 = cheapest URL 오설정이 랭킹 모달 전사
  실패를 낸 실사고(fail-loud 픽스). "최저가 1건만" 요건이 생기면 재검토(범위 밖(개정 2) 판단과 정합).
- **BNK 판매사 목록 external — 제프 선제 구현·배포 완료(2026-07-16, CRM 실측 검증)**:
  `GET /api/external/catalog/bnk-dealers?brand=`(X-API-Key — 기존 `EXTERNAL_API_KEY` 재사용,
  내부 `getBnkDealersForBrand` 동일 로직, 내부 라우트 무접촉·테스트 고정). **CRM 실측 3종**:
  ①성공 200 shape = **`{ok:true, dealers:[...]}`**(내부 `{dealers}`와 달리 ok 래핑 — 파서 계약
  확정, PR 서술 모호성 해소) ②키 없음 = 401 `{ok:false, errorCode:"AUTH_ERROR"}`(게이트 작동)
  ③딜러 데이터는 **수입 브랜드 위주**(BMW 3·벤츠 9·아우디 7 / 국산 기아·현대·제네시스 0) —
  UI 복원 시 빈 목록 처리 필수, 국산 0이 데이터 현실인지 시드 예정인지는 제프 확인 대기.
  **CRM UI 복원(반나절)은 이사님 수요 판단 대기**(질의 framing: "제프 준비 완료 — 복원할까요").
  **⚠️ 계약의 BNK-묶임 설계 리스크(유슨생 제기)**: 라우트명(`bnk-dealers`)·payload 필드
  (`bnkDealerName`)·엔진 소비(BNK 운용리스만)·부수 필드(`bnkSpecialPromotion`)까지 전층이 금융사
  고유명사 스코프 — "판매사(출고 딜러)"는 딜 차원의 일반 개념이라 타 금융사가 딜러별 금리를
  도입하면 계약 증식(`meritzDealerName`…) 리스크. **CRM 소비 계층은 lender-일반형 seam으로
  감싼다**(`fetchDealersForLender(lenderCode, brand)` — 제프 계약이 일반화되면 seam 본문만 교체).
  **🔄 제프가 자체 수정 진행 중(2026-07-16)** — 위에 박제한 라우트명·shape는 **유동**, 확정
  회신 수신 시 이 항목 갱신+재실측(200 shape·401 게이트·데이터 분포). **CRM 영향 0 실측 확인**:
  딜러 엔드포인트 소비 코드 0(주석 2건뿐)·릴레이 zod 딜러 필드 의도적 부재(strip이라 실수 유출
  경로도 차단)·휴면 필드(dealerType/dealer)는 타입 선언+기본값뿐 — 제프가 어떻게 바꿔도 현행
  CRM은 깨질 것 없음.
- **견적서 PDF**: 제프 활성 로드맵 없음 확정(원본도 자리표시자) — 대기하지 말고 자체 구현 권고
  수신. 구현 여부·우선순위는 별도 제품 결정.
