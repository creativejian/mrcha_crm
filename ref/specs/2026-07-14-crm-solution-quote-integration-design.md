...
- **BNK 판매사 목록 external — 제프 구현 완료 공유(2026-07-16, 수요 확정 전 선제 구현)**:
  `GET /api/external/catalog/bnk-dealers`(내부 `getBnkDealersForBrand` 동일 로직·동일 응답 shape,
  `createApiKeyMiddleware()` X-API-Key 게이트 — 기존 `EXTERNAL_API_KEY` 재사용). 에러 계약 =
  calculate/cheapest와 동일(401 AUTH_ERROR / 400 VALIDATION_ERROR / 500 INTERNAL_ERROR /
  200 `{ok:true, dealers:[...]}`). 내부 라우트 무접촉(게이트 미적용 유지, 테스트 고정).
  **⚠️ prod 미배포(CRM 실측 2026-07-16)**: 키 유무 무관 404 — 대조군 external calculate는 정상
  응답(빈 바디 → zod 400)이라 호출 방식 문제 아님, 라우트 미배포 확정. 배포 후 재실측 필요
  (성공 shape이 `{ok:true,dealers}`인지 내부와 동일 `{dealers}`인지 — PR 서술이 양쪽으로 읽힘 —
  확인 포함). **CRM UI 복원(반나절)은 이사님 수요 판단 대기** — 질의 framing은 "열어달라
  할까요"가 아니라 "제프 쪽 준비 완료 — 우리가 복원할까요"로 변경.
