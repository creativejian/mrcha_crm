...
- ~~**마킹 시 계약완료 전이 확인 넛지**~~ → ✅ **결정·구현(2026-07-21, 이사님 회신 ①ⓑ)**: 견적함 "계약 진행" 마킹과 고객 계약완료 전이는 실무상 한 이벤트의 두 기록인데 물리적으로 분리돼 있다 — 마킹만 하면 출고 큐 미진입(비계약완료 고객 = 유슨생이 김지안으로 걸린 케이스). pending 항목 15 질의에 이사님이 **① 확인 넛지 채택 + ⓑ 발주 경로를 그 자리에서 물어봄**으로 답변(딜러사계약중 고정 ⓐ 기각). 확정 계약:
  1. **발동 조건** = "계약 진행" 마킹 확정 직후(신규 마킹만 — 해제·재해제는 무넛지) ∧ 고객 진행 상태 1차 ≠ 계약완료(이미 계약완료면 생략). 같은 견적 액션 팝오버 안 인라인 스텝으로 노출.
  2. **수락** = 발주 경로 3종(`CONTRACT_ORDER_PATH_STATUSES` = 딜러사계약중·대리점발주중·특판발주중 — `customerStatusGroups.계약완료`의 발주 경로 부분집합, 파리티 테스트 잠금. 배정완료·출고완료는 발주 "경로"가 아니라 제외) 중 하나를 클릭 → `onWorkflowChange(customer.no, { statusGroup: "계약완료", status: <선택> })` — **기존 진행 상태 전이 경로 그대로**(App.updateCustomerWorkflow: 낙관 반영·chance 확정 자동 동기 Option A·PATCH·실패 롤백 전부 승계, 신규 저장 경로 0).
  3. **거절**("진행 상태 유지"·외부 클릭·Escape) = 아무 변경 없음 — 견적 마킹만 남는다(현행과 동일). 확인 기반이므로 S6 "결합 없음 원칙"과 양립(몰래 바뀌는 자동 조작 아님).
  4. **독립 낙관 2건** — 견적 마킹 PATCH와 상태 전이 PATCH는 서로 다른 낙관 갱신·롤백 단위(마킹이 성공하고 전이가 실패해도 마킹은 유지 — 각자 fail-loud 토스트). 넛지는 마킹 확정과 동시에 노출(마킹 PATCH 완료 대기 없음 — 마킹 실패 시 견적 쪽 롤백·토스트가 따로 알린다).
  5. **역방향 없음** — 마킹 해제가 진행 상태를 되돌리는 넛지는 미도입(계약완료 이탈은 드로어·목록 진행 상태 컨트롤 몫).
