...
⚠️ **메리츠 mileage는 런타임 유동이다.** 활성 워크북 DB 파생이라 제프 DB 장애 시 **메리츠 mileage만 `null`로 강등된 200**이 온다(500 아님 — 제프가 CRM fail-open과 맞물리도록 의도). CRM은 `null`을 미확정으로만 해석하므로 자연히 게이트가 풀린다. **같은 금융사의 지원집합이 응답마다 다를 수 있음**을 전제로 설계한다(캐시를 영구 보관하지 않는 근거).
...
| D5 | 캐시 = 세션 단위(새로고침 시 갱신) | `staff.ts` 선례. 매트릭스가 1KB 미만이라 무겁지 않고, 메리츠 유동성(§2 ⚠️)상 영구 보관은 부적절. |
...
- 두 경우 모두 **저장된 카드(`isConditionSaved`)는 이미 전체 disabled**라 무관.
