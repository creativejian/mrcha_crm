...
- **역방향 게이트(조건 → 금융사 목록 필터)** — 유슨생 판단 2026-07-21. "12개월을 고르면 MG가 목록에서 빠져야 하지 않나"는 대칭성 직관이 자연스러워 **재부상하기 쉬운 제안**이라 근거를 남긴다.

  실효는 축마다 다르다 — 기간은 12·24개월 모두 **MG 하나**뿐이지만, 약정거리는 25,000km에서 4사가 빠져 우리카드만 남을 만큼 크다. 그럼에도 기각한 이유 3가지:

  1. **미확정 3사 때문에 필터가 반쪽이다.** 산은·iM·농협은 `null`이라 걸러낼 근거가 없어 **항상 남는다.** 12개월에선 "MG만 사라지고 7사는 그대로"라 필터보다 *"MG가 왜 없지"*로 읽히고, 35,000km에선 5사가 빠지고 **미확정 3사만 남는 역설**이 된다 — 그 3사가 바로 지금 값을 지어내는 중인 곳이다(파트너 Phase B 미착수).
  2. **수기 작성이 막힌다.** 워크벤치는 솔루션 조회 없이 수기로도 쓰는 화면이다(작성방식 3종). 기간 버튼과 달리 **금융사는 견적의 정체성**이라, 목록에서 빠지면 그 금융사 견적을 손으로도 못 쓴다. 파트너가 계산을 못 해주는 것과 상담사가 적는 것은 별개다.
  3. **순서 의존(막다른 골목)이 생긴다.** 지금은 어느 순서로 골라도 도달 가능하다. 역방향까지 막으면 "MG를 쓰려면 기간을 먼저 되돌려야" 한다.

  현행도 정보 손실은 없다 — 12개월 + MG를 고르면 폴백이 60개월로 옮기고 **사유를 토스트로 알린다**(D3).
