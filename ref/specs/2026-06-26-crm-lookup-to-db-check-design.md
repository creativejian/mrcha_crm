...
5. **방어 부재의 실증:** `test:server`가 검증을 우회하는 query 직접 호출로 garbage 4건(`없는분류`/`없는종류`/`존재하지않는종류`/`비교 견적`)을 master DB에 적재했다. lookup/zod(라우트 검증)는 이 우회 경로를 못 막는다. **DB CHECK였으면 처음부터 거부됐다.**
...
1. **`test:server` 잔재 경로 차단** — garbage를 만드는 테스트를 찾아(검증 우회 query 직접 호출 라운드트립) 정식값을 쓰거나 정리하도록 수정. CHECK 적용 후 그 INSERT는 DB가 거부하므로 어차피 수정 필요.
