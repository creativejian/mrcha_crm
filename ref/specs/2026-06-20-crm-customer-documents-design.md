...
김민준(`CU-2605-0020`) 상세 drawer의 **서류함**을 메모리(objectURL)에서 실제 영속 저장으로 전환한다. 파일(신분증·소득증명·사업자등록증 등 이미지/PDF/오피스 스캔본)을 Supabase **private 버킷**에 올리고, 메타를 `crm.customer_documents`에 기록한다. 업로드·분류수정·순서변경·삭제·미리보기/다운로드를 DB/Storage와 연동한다. CRM 도메인의 첫 **바이너리 영속(파일)** 경로를 세운다.

## 범위

**포함**: 김민준 서류함의 업로드/분류수정/순서변경/삭제/미리보기/다운로드 DB·Storage 연동. private 버킷 1개. 백엔드 multipart 수신 + service_role Storage 처리. signed URL 발급.

**제외(후속/별도 사이클)**:
- 김민준 외 고객 서류함 — 다른 고객 상세는 아직 구 일반 레이아웃(읽기). 서류함 UI 자체가 없어 레이아웃 작업이 선행돼야 함.
- OCR/AI 문서 판독, PDF 원본 병합 — 후속(라이브러리/서버 처리).
...
1. **업로드 경로 = 백엔드 경유.** Hono가 multipart로 파일을 받아 **service_role** 키로 Storage 업로드 + `customer_documents` row 생성을 한 핸들러에서 처리. 이유: 기존 JWKS 인증 게이트 재사용(권한 일관), 업로드+메타 원자성, 프론트에 민감 버킷을 publishable key로 노출하지 않음, 새 RLS 정책 불필요(crm 도메인은 service가 직접 접근하는 기존 패턴과 일관).
2. **적용 범위 = 김민준 전용.** 쓰기 #1·#2와 동일하게 `KimDocumentContent` UI에만 실 연결. 단 백엔드 API/버킷/테이블은 고객 불문 일반적이라 다른 고객 확장 시 재사용 가능.
3. **파일 제약 = 이미지 + PDF + 오피스문서, 파일당 ≤ 20MB.** 허용 MIME: `image/*`, `application/pdf`, 오피스(xlsx/docx/pptx 및 구형 xls/doc/ppt). 미리보기는 이미지·PDF만, 오피스는 다운로드만.
...
- **service_role 키 발급/등록은 유슨생이 직접** 수행:
  - 로컬: `.env.local`에 `SUPABASE_SERVICE_ROLE_KEY=...` 추가(`.env.example`에도 키 이름만 추가).
  - CF: `wrangler secret put SUPABASE_SERVICE_ROLE_KEY` (production+preview). 변경 후 재배포 필요(기존 배포는 옛 값).
...
- `getServiceClient(env)` — `@supabase/supabase-js` `createClient(SUPABASE_URL, SERVICE_ROLE_KEY, { auth: { persistSession: false } })`. CF/로컬 키 해석.
...
- **미리보기/다운로드**: objectURL 제거. 미리보기/다운로드 시 `getDocumentUrl`로 signed URL 받아 `<img>`/`<iframe>`(이미지·PDF) 또는 anchor 다운로드. 오피스는 다운로드만.
...
[Hono] auth 게이트 → MIME/size 검증
   ...
   → uploadObject(service_role) → addDocument(row, sortOrder=max+1)
...
- service_role 키는 **백엔드 전용**. 프론트 번들·응답에 절대 노출 금지. signed URL만 프론트로.
