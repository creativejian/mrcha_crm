...
## 7. 후속 — 실시간 상담 콘솔 권한 감사 (2026-07-21 심야)

§6의 후속(#307이 기각한 `GET /api/staff` 차단의 선행 조건 확인). **CRM 코드 변경 없음 — 수정할 것이 없었다.**

### 구조 — 채팅 쓰기는 서버 라우트를 타지 않는다

`assignSession` · `takeOverSession` · `returnSessionToAi` · `sendStaffMessage` 4종 전부 **supabase-js 직결**(`client/src/lib/chat.ts`)이다. 따라서 **전역 `dealerWriteGate`(#220)가 원리적으로 닿지 않고**, `ChatPage`에도 role 게이트가 없다(`roleTab`은 운영 배지 `canManage`에만 쓰인다). **RLS가 유일한 방어선**이다.

### 판정

- **dealer 차단 = RLS가 이미 담당** ✅ `user_role` enum엔 `dealer`가 있으나 `chat_sessions`·`chat_messages` 정책 배열은 `staff·manager·admin`뿐이라 조회조차 안 된다. 사이드바도 dealer는 별도 메뉴라 `/chat` 진입점이 없다. → **#220의 사각지대가 아니다.**
- **staff의 세션 배정 자유 = 유지(막지 않는다)** `chat_sessions` UPDATE가 staff 전체에 열려 아무 세션이나 아무에게나 배정할 수 있지만, **SELECT도 전체 허용이라 넘긴 뒤에도 계속 보인다** — #307을 막은 근거(넘기면 스코프에서 사라져 복구 불가)가 여기엔 성립하지 않는다. 대기 큐에서 세션을 주고받는 것은 정상 업무.

### 앱 팀 제보 (우리가 못 고치는 영역)

`chat_messages` INSERT 정책의 WITH CHECK가 **role만 보고 컬럼 값을 검증하지 않는다** → `staff_id` 사칭·`is_user: true` 위조·타 고객 대화 삽입이 정책상 가능. 내부 계정 전제라 심각도는 낮게 본다. 앱 소유 테이블·앱 소유 RLS라 제보만 한다 — `ref/2026-07-21-app-chat-rls-report.md`.

**실 INSERT로 확증하지 않았다** — 이 테이블은 `notify_staff_chat_message`로 고객 FCM 푸시를 쏘므로(AGENTS.md 알림 4테이블) 확증 이득보다 실수 위험이 크다. 정책 정의만으로 명확하다.

### 관찰(무수정)

`/chat` 라우트에 role 게이트가 없다 — dealer가 URL을 직접 치면 페이지는 뜨고 RLS가 데이터를 막아 **빈 화면**이 된다. dealer 실계정 0명 + 딜러 화면 미설계라 지금 손대지 않는다. **재론 트리거 = 딜러 화면 설계 착수 또는 dealer 실계정 생성.**
