...
**③ 실기가 부수 결함 6건을 찾았다.** ⓐ`#343` `deposit_type='none'`(무보증) 라벨 누락 — 07-17부터 raw "none" 노출 ⓑ`#345` 니즈 카드가 `additional_request`를 렌더 안 함(#340 누락) ⓒ`#344` **카카오 OAuth가 앱 도메인으로 튀던 것** ⓓ`#346` 계약·출고 목록이 니즈를 계약 차량·구매방식처럼 표시 ⓔ`#347` 출고 관리 3열에 너비 규칙이 없어 `table-layout:fixed`가 균등 분할(⚠️ 폭은 **빈 상태가 아니라 값이 들어찬 최장 케이스**로 잡을 것 — 한 번 틀렸다) ⓕ`#348` **실체 없는 "계약 관리 창"으로 안내**하며 계약 진행 견적 수정을 막던 것(spec §113이 stale 문구로 이미 기록해 뒀다) → 경고 후 수정 허용.
