...
- **🔴 미해결(인프라, 결정 필요): prod Gemini "User location is not supported"(400 FAILED_PRECONDITION)** — CF Workers 콜로 라우팅에 따라 Gemini 무료 티어가 리전 차단(2026-07-03 저녁부터 지속, 낮에는 정상 — 간헐). 업무 AI prod가 라우팅에 따라 불능. 로컬 키(한국 IP)는 정상. 해결 후보: ①**키에 billing 연결(유료 티어 — 위치 제한 해제, 권장·이사님 결정)** ②Vertex AI 전환(리전 고정, 변경 큼) ③고정 리전 프록시. **#143의 dbHold prod 실측 검증도 이 이슈 해소 후 진행**(코드는 리뷰+유닛 통과).
