...
- **master Supabase 직접 통합 — A2 Phase A/B/C 구현 완료.** schema 3분할(public 앱 19 / catalog 차량 9 / crm 운영 8)이 prod 실재. `DATABASE_URL`=master.
- **Phase A**(PR #21, `aebaf57`): `crm` 8테이블 master 생성.
- **Phase B/C**(브랜치 `feat/a2-phase-bc-catalog-adopt`, 미머지): catalog adopt + 차량 read master 직결(거울 `deleted_at` 필터 제거) + 거울 sync 폐기 + `crm.quotes→catalog` FK(SET NULL).

## Phase B/C 작업 (브랜치 feat/a2-phase-bc-catalog-adopt)
...
- **PR 올리고 머지**(feat/a2-phase-bc-catalog-adopt → main).
