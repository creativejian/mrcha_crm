...
4. **[소형·미착수] `formatPhone` 10자리 처리** — 3-3-4로 잘라 서울 유선번호가 `0212345678` → **`021-234-5678`**로 잘못 나온다(`client/src/lib/phone-format.ts`, 화면·AI 공용이라 **양쪽에 영향**). 실 데이터가 전부 11자리 휴대폰이라(10자리 **0건** 실측) 실영향 0이라 `#336` 범위 밖으로 뒀다. 고칠 땐 `02`만 2자리로 끊으면 되고 **화면 표시도 함께 바뀐다**. ⚠️ 그때까지 이 함수 테스트에 유선번호를 넣지 말 것(잘못된 동작을 박제한다 — `#336`에서 실제로 그럴 뻔해 픽스처를 휴대폰으로 교체했다).
