...
- 🔴 **바로 착수 = 배치 16 감사 수정 6항목**(유슨생 승인 2026-08-06 · 집에서 진행). 판정 SSOT =
  **`ref/plans/2026-08-06-crm-refactor-batch-16.md`**(그 파일만 읽으면 된다 — 감사 재실행 불필요).
  ①P1 안전장치(`listRetentionDueCustomers`에 `AND app_user_id IS NULL`) ②**WHERE 절 tripwire**
  ③D+3/D+5 픽스처 조이기 ④`#436` 실적 귀속 테스트 ⑤정산 `loaded` 게이트 ⑥스테일 주석 5건.
  ⚠️ **canonical 축·C2 디바운스·P1ⓑ(정책)는 후속으로 뺐다** — 그 파일 "후속" 절 참조.
...
- 🔴 **진짜 발견은 개별 결함이 아니라 CI 구조다** — 권한·파기·돈의 **서버 그물이 전부 `test:server` 전용**
  ...
  (전 고객 삭제 · 전 고객 수렴 파기 · 전 딜러 제안 삭제). 두 변이 앵글이 독립적으로 같은 결론에 도달.
...
- 유일한 실동작 결함 = **B 보존 고객 재연결 시 파기**(CONFIRMED). 앱 회신 §8이 재연결을 **정상 경로로 명시**해
  "도달 불가" 반박이 실패했다. 현재 노출 0(B 분류 미사용). `#459`·배지 축 5연타는 archive로.
