...
## Current Focus (2026-06-19) — 라우팅 딥링크 구현 완료(브랜치 `feat/crm-customer-detail-deeplink`, PR/머지 대기). 다음=고객 쓰기

- 인증+CF 배포 + 코드리뷰 후속(#43~#45) + **CRM 고객 읽기(#46)** + **mc-master UX/성능(#47~#52)** + **고객 상세 읽기 연결(#51)** 전부 main 머지. **crm.mrcha.app 라이브**.
- **방금 완료(브랜치): 고객 상세 라우팅 딥링크** — 선택 고객이 URL이 source of truth(`/customers?customer=CODE` 드로어 · `/customer-detail/:code` 페이지). 새로고침·북마크·링크공유·브라우저 뒤로가기 동작. 설계/플랜 `ref/{specs,plans}/2026-06-19-crm-customer-detail-deeplink*.md`.
- **다음 작업: 고객 쓰기** — 상태/계약가능성/상담메모/니즈/할일/일정 인라인 수정을 DB PATCH로(현재 프론트 상태만). + advisor 배정·profiles 연동. 이후 견적(quotes). + (별도 설계 사이클) enum/lookup 도메인 정리 — 업무 어휘(진행상태·유입경로·서류종류 등)는 lookup, 기술 내부값은 enum.

## 완료 (2026-06-19)

- **고객 상세 라우팅 딥링크 (브랜치 `feat/crm-customer-detail-deeplink`, PR 대기)**: 선택 고객을 React 상태(`selectedCustomerNo`/`customerDetailPanelOpen`)가 아니라 **URL이 single source of truth**로. 2커밋 — ①`lib/customer-route.ts` `customerCodeFromLocation(pathname,search)` 순수함수(+테스트 7): `/customer-detail/:code`→path code, `/customers?customer=`→쿼리값. ②`App.tsx` — 상태 2개 제거+버그폴백(`?? customers[0]`) 제거, `customersLoaded` 추가, `selectedCode`/`selectedCustomer`/`isDrawerOpen` URL 파생, 라우트 `/customer-detail/:code`(+`/customer-detail`→리다이렉트, 로딩/없음 처리), 핸들러 navigate화(드로어 열림=`?customer=` push, 재오픈=replace), ESC/백드롭=navigate. `CustomerDetailPage`/`CustomerManagementPage` 무변경(값 출처만). 보너스: 브라우저 뒤로가기 자연 동작. **검증**: typecheck 0·lint 0·test:unit 124·build OK. e2e는 인증 세션 필요라 새로고침/공유 **수동 확인**.
...
- ~~고객 상세 읽기 연결~~ ✅ #51 머지. ~~라우팅 딥링크~~ ✅ 완료(브랜치 `feat/crm-customer-detail-deeplink`, PR 대기) — 위 "완료" 참고.
- **고객 쓰기 (1순위)** — 진행상태/계약가능성/상담메모/니즈/할일/일정 인라인 수정을 DB PATCH로(현재 프론트 상태만). 김민준 상세는 이미 in-scope state가 detail 파생이라, 저장 핸들러에 PATCH만 붙이면 됨(낙관 갱신 + 실패 롤백 고려). + advisor 배정·profiles 연동(목록 담당 현재 "미배정"). 읽기 설계의 "범위 외" 참고.
