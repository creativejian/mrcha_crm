...
- ~~타깃 렌즈 배치~~ **완료(후속 PR 3건까지 종결)** → 위 요약. **잔여 = 죽은 상태 modifier 4종**
  (no-op 확인·제거/CSS복구 중 원 의도 불명) · 구조 개선 후보 = `useQuoteList.quotes` 미재동기화.
  ⚠️`#416`·`#417` **prod 실기는 유슨생 직접 확인 예정** · 검증용 딜러 제안(550e 200/220/240만,
  채택 전이라 확정 할인 무영향) 정리 여부 판단 대기.
- ~~CF 잔여 권고~~ **전부 종결**: rate limiting(IP당 5회/10초·실측 429) · AI Gateway 보류 ·
  프리뷰 Access 종결.
