...
- 🟡 **보류 = 버전 표시·릴리스 체계**(2026-08-06 유슨생 "고민 좀 더"). **설계는 다 나와 있다** —
  `ref/plans/2026-08-05-crm-versioning-release.md`(방향이 한 번 뒤집힌 경위·조사 결과·대시보드
  체크리스트 포함). 재개 시 **처음부터 다시 논의하지 말 것.** 요지: 배포 트리거를 **태그**로 옮겨
  prod 코드 = 화면 버전을 항상 일치시킨다 · Workers Builds는 태그 미지원이라 **Actions `wrangler
  deploy`**(Deploy Hook은 브랜치 HEAD를 빌드해 요구 불충족) · 스테이징 없음(로컬 확인).
