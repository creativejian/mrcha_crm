...
- ✅ **prod 실기 완료**(`https://mrcha-crm.pages.dev`, 7종 × 3회) — **로컬과 동일 12/12 · 회귀 2종 정상 · 도구 경로 번호 노출 0 · 대화 기록 잔재 0.** prod는 **Gemini 리전 프록시(#144) 경유**라 프롬프트가 같아도 결과가 같다는 보장이 없어서 이 실기가 필요했다. 부수 관찰: **AI가 번호 포맷을 임의로 바꾼다**(`010-5486-8279` 형태 — 자릿수 동일이라 환각 아님·표기만 흔들림).
