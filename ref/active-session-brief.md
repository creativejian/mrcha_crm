...
## Current Focus (2026-06-19) — CRM 고객 읽기 + mc-master UX/성능 정리 완료. 다음=고객 상세 읽기 연결

- 인증+CF 배포(이전 세션) + 코드리뷰 후속(#43~#45) + **CRM 고객 읽기 DB 연결(#46)** + **mc-master UX/성능(#47~#50)** 까지 전부 main 머지. **crm.mrcha.app 라이브**.
- **다음 작업: 고객 상세 읽기 연결** — `CustomerDetailPage`가 아직 목업/하드코딩(김민준 시범)을 표시 중. `getCustomer`/`fetchCustomer` 백엔드는 #46에서 준비됐으나 프론트 미사용. **B안 권장**: 헤더(`CU-…·날짜`)·상세구매조건·메모/일정/할일 등 명백한 하드코딩만 실데이터로, 레이아웃 일반화는 별도. 이후 고객 쓰기·견적. 읽기 설계 `ref/specs/2026-06-19-crm-customers-read-design.md`.

## 완료 (2026-06-19 — 전부 main 머지)
...
- **고객 상세 읽기 연결 (1순위)** — `CustomerDetailPage`가 `fetchCustomer`(=`GET /api/customers/:id` getCustomer, 자식 tasks/schedules/memos/documents/consultations 포함)로 헤더·상세구매조건·메모/일정/할일을 실데이터로. 현재 김민준 시범 하드코딩. B안=명백한 하드코딩만, 레이아웃 일반화는 별도. (brainstorming→spec→plan 사이클)
- **고객 쓰기** — 진행상태/계약가능성/상담메모/니즈 인라인 수정을 DB PATCH로(현재 프론트 상태만). + advisor 배정·profiles 연동(목록 담당 현재 "미배정"). 읽기 설계의 "범위 외" 참고.
