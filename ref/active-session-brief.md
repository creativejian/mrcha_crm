...
- **🔴 미해결(인프라, 결정 필요): prod Gemini "User location is not supported"(400 FAILED_PRECONDITION)** — CF Workers 콜로 라우팅에 따라 Gemini 무료 티어가 리전 차단(2026-07-03 저녁부터 지속, 낮에는 정상 — 간헐). 업무 AI prod가 라우팅에 따라 불능. 로컬 키(한국 IP)는 정상. 키는 이미 유료 티어(충전됨) — 원인은 billing이 아니라 **HKG 콜로 라우팅**(cf-ray 실측: KIX 200·HKG 500·NRT 200 — 홍콩은 유료여도 Gemini 차단 지역). **결정(유슨생, 2026-07-03): A안 = Gemini 호출을 Supabase Edge 프록시 경유로 진행**(리전 고정 — 앱 ai-analyst가 Supabase Edge에서 Gemini SSE 실사용 중이라 실증됨. 얇은 릴레이 함수 + CRM gemini-embed/generate URL 스위치). **#143의 dbHold prod 스트리밍 실측 검증도 이 작업 후 마무리**(논스트리밍 200 2회 실증, 코드는 리뷰+유닛 통과).
