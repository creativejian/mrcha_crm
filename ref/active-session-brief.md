# Mr. Cha CRM Active Session Brief

> **자동 로드 · 60줄 이하 유지**(AGENTS.md). 과거 로그 = `ref/session-archive.md` · 지속 결정 = `AGENTS.md` · 설계 = `ref/specs/*` · 장기 상태 = `ref/current-working-state.md`.

Last updated: 2026-08-05

## 지금 상태

main 전량 green(CI 8단계). unit **1437** · pure **290**. 정산 축 종결 후 `#451`~`#459` 완료
(팝오버 분리 · mc-master UX 2 · 배지 축 5 + 조회 합치기). ✅ **앱 팀 회신·제약 적용 완료** —
`catalog.trims`가 `(model_id, name, model_year)` UNIQUE NULLS NOT DISTINCT다(막혀 있던 트림
등록이 열렸다. 상세 = `ref/2026-08-05-app-trim-model-year-unique-request.md`).

## ▶ 다음

- 🔴 **바로 착수 = 버전 표시 + 릴리스 체계**(유슨생 지시 2026-08-05, 새 세션). 사이드바 로고 블록에
  `v1.0.0`을 띄우고, **릴리스 시점을 사람이 정한다**(커밋마다 버전이 오르면 팀장·직원 노이즈).
  설계 SSOT = **`ref/plans/2026-08-05-crm-versioning-release.md`** — 결정 6·함정 4·미결정 3이 거기 있다.
  요지: 버전 SSOT는 `package.json`(git describe 금지 — CF가 자기 환경 빌드) · **배포는 main push
  유지**(바꾸지 않는다) · 릴리스는 **release-please Release PR 머지** · 화면 버전은 "팀에 알린 단위"라
  prod 코드가 앞서는 게 정상(hover에 SHA). ⚠️ release-please 설정은 **context7로 최신 확인** 후 작성.
- **잔여 2건(머지는 끝났다 — 실행·육안만)**: ⓐ**김지운(`jiandolce@gmail.com`) 탈퇴 테스트 미실행**
  (견적 20·**앱카드 19** — 카드 회수 경로 검증) ⓑ`#445` 배너 **육안 미확인**. **ⓐ를 하면 ⓑ가 딸려 온다**.
- **🔴 시범 고객·풀세트 시드 재지정**(상세 드로어 작업의 선행 — 김민준 삭제로 공백). **이사님 결정 영역**.
- ⚠️ **트림 등록 후 "고유번호 할당" 실행**이 운영 절차다(`auto_mc_code`가 BEFORE UPDATE 전용 —
  INSERT만으론 mc_code가 NULL이고 파트너 비교에서 빠진다). 현재 미부여 **148/1811**.
- 리팩토링 대기 후보 없음(`#451`로 소진). 다음 큰 파일은 트리거가 없어 착수하지 않는다.

## 직전 세션 (08-05 밤 2 · 유슨생)

- **`#459` 배지 조회 합치기(A+B)** — 배지 축의 마무리. **✅ 실기 육안 확인 완료**(승인 시 사이드바·
  브랜드·헤더가 **동시에** 카운팅). 실측 **큐 3→1 · gaps 2→1**. ⚠️ 착수 시 브리프 계산이 틀렸다 —
  "큐 2"가 아니라 **3**이었다(헤더 대기열 버튼 인스턴스 누락). 서버 변경 0.
- 설계 셋(코드 주석이 상세): ①**세대(epoch) 가드** — 무조건 합치면 승인 직전 출발한 조회에 승인 직후
  재조회가 올라타 **승인 전 숫자**를 받는다. ②**broadcast는 signals가 한 번만 구독**해 되뿌린다.
  ③대기열 응답은 **모듈 스냅샷 한 벌** — B가 폴링 소유권을 옮기는 순간 없으면 새 불일치가 생긴다.
- 🔴 **교훈: 상태를 모듈로 올릴 땐 인스턴스 state가 하던 가드도 함께 옮긴다.** effect cleanup이 막던
  **응답 순서 역전**이 빠질 뻔했다 → `queueSeq`로 이관. 그물 5종은 **변이 4종 주입**으로 실제로
  빨개지는 것을 확인했다(정적 통과는 증거가 아니다).
- 🟡 행위 변경: `enabled:false`인 대기열 훅은 남은 캐시 대신 `rows: null`(미로드). 현 소비처 영향 0.
- 배지 축 앞선 5연타(`#451`~`#458`)와 08-05 낮 세션은 archive로.

## 대기

**이사님** = `ref/director-pending-confirmations.md` **18건**(항목 14·16~32 — **31·32 신설**:
정산 목록 대상 기준=계약 확정일 / 열 이름 "실입금액". 같은 화면이라 **묶어서** 여쭐 것) + Supabase
**"EXCEEDING USAGE LIMITS" 배지**(08-01) 플랜 + **D+5 최악 6일차** SLA 해석.
💡 회신 때 **🟡 하드 삭제 확장(`#440`)을 항목 28**(AI 기록에 번호 잔존)**과 묶을 것** — 같은 축·미등재.

## Boot

`AGENTS.md` → 이 파일 → `git status -sb` · `git log --oneline -5`. 상세: 정산 = spec §6a · 보존 계약 = AGENTS.md.

## 세션 마무리 규칙

**교체**(누적 금지 · 이전 요약은 `ref/session-archive.md` 맨 위로). 행위 변경은
`ref/director-pending-confirmations.md` 등재(**유슨생 현장 승인은 등재 없이 박제** · 이사님 확정
설계를 뒤집는 건만 등재 · 신설 시 그 파일 롤업 2곳도). 상세 규칙 = AGENTS.md "Handoff Documents".
