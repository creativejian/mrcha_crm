# Mr. Cha CRM Active Session Brief

> **자동 로드 · 60줄 이하 유지**(AGENTS.md). 과거 로그 = `ref/session-archive.md` · 지속 결정 =
> `AGENTS.md` · 설계 = `ref/specs/*` · 장기 상태 = `ref/current-working-state.md`.

Last updated: 2026-08-08

## 지금 상태

main 전량 green(CI 8단계) = `c6a5287`. 08-08 오전 8 PR(hermetic 이관·리뷰 이행) + 오후 4 PR.
빠른견적 4단계는 08-07 전 구간 배포 완결(계약 = `ref/2026-08-07-app-quote-request-ready-for-send-reply.md`)
— 실기만 이사님 대기. 오전 세션 상세 = `ref/session-archive.md` 맨 위.

## 직전 세션 (08-08 오후 · 유슨생) — 4 PR

- **`#475` 관리중 고객이 두 업무함에 동시 노출되던 것 해소** — 미배정 + "관리중"이 상담 필요·보류/
  이탈 **양쪽에** 떠 어느 쪽도 소진되지 않았다(9→6명). `#260`이 배정 축만 고치고 stage set은 목업
  시절 것을 둔 잔재. 🟡 `director-pending-confirmations` **항목 33**에 판정·기각안·반증 실측 전문.
- **`#476` 키보드 단축키**(신규) — `?`로 우측 시트, 전역 + 화면 이동. 설계 SSOT =
  `ref/specs/2026-08-08-crm-keyboard-shortcuts-design.md`. 🔴 **꼭 알 것**: ①**판정은 `event.key`가
  아니라 `event.code`(물리 키)** — 한글 모드에서 `g`는 `key="ㅎ"`이라 key로 읽으면 시퀀스가 죽는데
  **영문 테스트는 전부 통과**한다(되돌리지 말 것 · 한글 실기 완료) ②**`lib/nav-visibility.ts` =
  가시성 SSOT**(사이드바 공유 · 새 메뉴는 여기만) ③**목적지 없는 메뉴엔 키를 주지 않는다**.
- **`#477` 힌트 노출** — 사이드바·팝오버 = 라벨 옆 인라인(펼친 상태만) · 상단바 아이콘 = 버블.
  ⚠️ **규칙: 같은 목적지에 새 키를 주지 말 것**("실시간 상담 요청"처럼 리라우트 진입점은 목적지
  화면의 키를 안내 — `advisorAssignmentModes` 3번째 원소가 "표시 이름 ≠ 레지스트리 어휘"를 드러낸다).
- **`#478` 설정 단축키 `S`+X 6종 · 프로필 `O P` · 무결성 tripwire** — 계정 팝오버 메뉴(admin 전용).
  접두를 나눈 근거는 알파벳 여유가 아니라 **이니셜**(조직 O·딜러 D·지식 K·AI A·상담운영 H가 G에서
  전부 점유). ⚠️ `S` 단독으로 팝오버를 못 연다(접두라 1.5초 대기) → 열기는 `O P`.
  🔴 그 과정에 **`G O` 결함**이 드러났다 — 사이드바 "팀원 관리"는 화면 없는 스텁인데 키는
  `/org-members`(실제 이름 "조직 / 구성원")로 갔다. `S O`로 옮기고 스텁은 SIDEBAR_ONLY 등재.
  🔴 **하루에 라벨·목적지 불일치가 3번** 나서(G A/G I 중복 · 리라우트 진입점 · G O) tripwire를 세웠다
  (`shortcut-integrity.test.ts`): 라벨 리터럴 드리프트(소스 스캔) · **같은 목적지에 키 둘 금지** ·
  경로 실재성 · path/action 배타. 세우자마자 **구분자 혼용**을 잡았다 — `" · "`는 **부모·자식 전용**이다
  (파리티 정규화가 그 규칙에 의존). SSOT 3축 = 표기 문자열 `keyboard-shortcuts.ts` · 표시 스타일
  `styles/shortcut-hint.css`(신설, 인라인 8줄 2벌 중복 해소) · 가시성 `nav-visibility.ts`.
- 변이 실증 **14종** 전부 RED 후 수동 역편집 원복 · 워킹트리 무손상(누적 29종).

## ▶ 다음 (미확정 — 트리거 없음)

**hermetic 이관 후보 소진**(남은 registry 47은 실 데이터·실 Gemini·실 조직이 검증 대상이라 옮기면
의미를 잃는다). 리뷰 보류분(무잠금 경합·TOCTOU·cross-brand)은 기록만. 후보 1: **Topbar 실적 배지가
하드코딩 목업**(`Topbar.tsx` "2026년 5월 · 86대 · 48.7억")인데 같은 3지표 실데이터 조립기
(`lib/admin-hero.ts`, `#428`)가 이미 있다 — 배선만 남았고 당월 0건 표시가 결정 사항이다.

## 실기 대기 (이사님)

2·3단계 최초 1회 · iOS 문구 · 4단계 구매방식/차량 조각 + **v39 timing 실측**(`[send-push] timing`).
이상 시 경계 좁히기: DB timestamp → CRM tag 요청 → App consumer → iOS 표시 순.
✅ 단축키 한글 모드 실기는 **완료**(08-08 유슨생 — `G H`·`?`·`⌘K` 정상). `event.code` 판정 유지.

## 기존 대기

- ⚠️ CI 그물 `delete-where-guard`(`#460`) — 파기 `.delete(테이블)`은 `.where()` 필수(ALLOW에 사유 등록).
- 김지운 탈퇴 테스트(견적 20·앱카드 19) + `#445` 배너 육안 · 🔴 시범 고객 재지정(이사님 결정 영역).
- 트림 mc_code 미부여 198/1869(08-06 실측·차단 0) — 트림 등록 후 "고유번호 할당"이 운영 절차.
- `ref/director-pending-confirmations.md` **19건**(08-08 항목 33 신설) + Supabase usage 배지 + D+5 SLA 해석.
- 버전 표시·릴리스 체계 보류 — 재개 시 `ref/plans/2026-08-05-crm-versioning-release.md`부터(재논의 금지).

## Boot / 세션 마무리 규칙

부팅: `AGENTS.md` → 이 파일 → `git status -sb` · `git log --oneline -5`.
마무리: **교체**(누적 금지 · 이전 요약은 `ref/session-archive.md` 맨 위로 · 60줄 이하). 행위 변경은
`ref/director-pending-confirmations.md` 등재(유슨생 현장 승인은 박제만 · 이사님 확정 설계를 뒤집는
건만 등재). 상세 = AGENTS.md "Handoff Documents".
