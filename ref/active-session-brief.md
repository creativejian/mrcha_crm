...
Last updated: 2026-07-04 (백로그 6 sse-liveness 추출 완료 — #153 머지, AI 리팩토링 백로그 잔여=B 트리거 3건뿐)
...
- 다음 후보: ①**역할 기반 scope 실제화(보류 — 나중에)**: **팀 개념 없음 확정(유슨생 확인, 2026-07-03) — "팀장"은 팀 관리가 아니라 권한 레벨만 상이.** 따라서 crm.staff/팀 테이블·팀 모델링 불필요, scope는 "admin/팀장=전체, 상담사=본인 담당(advisor 매칭)" 역할 분기로 축소(advisor_id uuid 병기 + `resolveCustomerScope` seam 본문 교체 + 보류된 seam predicate). staff 계정 0이라 실사용 압력 없음 → 데이터화 이후로 ②**데이터화 마감+kim 리네임 슬라이스 — 설계 확정(2026-07-04)**: **이사님 점진 일반화 컨펌 완료**(유슨생 전달). 실태 조사 결과 레이아웃 일반화·자식 CRUD DB화는 #118~#122에서 이미 완료 — 실범위 = 잔여 mock 4곳(타임라인 consultations 병합·관리 상태 SQL 파생+bucketing·장식 mock 2곳 제거) PR1 + kim 리네임(TS 심볼+파일명만, CSS 클래스 보류) PR2. spec: `ref/specs/2026-07-04-crm-detail-datafication-design.md`.
