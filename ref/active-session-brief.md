...
## Current Focus (2026-06-20) — #3 서류(파일 업로드) 코드 완성, PR `feat/crm-customer-documents` 올림(미머지). 유슨생 대기: secret key·버킷·수동검증. 다음=enum/lookup 설계 또는 #4 견적
...
- ~~읽기 #51~~ · ~~딥링크 #53~~ · ~~쓰기 #1 #54~~ · ~~쓰기 #2 #55~~ · ~~전화번호 #56·#57~~ · ~~상세 로딩 perf~~ ✅ main 머지 · ~~서류 #3~~ ✅ PR `feat/crm-customer-documents`(미머지, 위 "완료" 참고).
...
- (2026-06-20 서류 #3) `typecheck` 0 · `lint` 0 · `test:unit` **125** · `test:server` **46**(`bun test --env-file=.env.local`; 기존 39 + 서류 라우트 4 + 검증유틸 3) · `build` OK. 서류 업로드/미리보기 **브라우저 수동검증은 유슨생 대기**(secret key·버킷 필요, 인증 세션).
