...
- **PR #61~#77 머지**(코드베이스 전체 분석 5에이전트 기반). 죽은코드·도메인상수·lib/http 통일·백엔드 run()·**Topbar usePopoverDismiss(#72)**·**CSS brand 토큰화(#73**, `#5836ff` 399곳→`var(--brand)`, 검증=정적 diff로 시각변화0)·**거대파일 분해 1~4차(#74~#77)**: `CustomerDetailPage` **6099→5706줄**, 도메인별 타입+상수+헬퍼를 `lib/kim-detail-utils`·`kim-schedule`·`kim-status-fields`·`kim-popover-frames`로 분리(+단위테스트 42).
