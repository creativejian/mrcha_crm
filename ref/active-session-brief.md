...
## Current Focus (2026-06-19) — CRM **고객 읽기** DB 연결 완료. 다음=고객 쓰기 or 견적

- 인증+CF 배포(이전 세션) + 코드리뷰 후속 정리(#43~#45) + **CRM 고객 읽기 DB 연결(#46)** 까지 전부 main 머지. **crm.mrcha.app 라이브**.
- DB 연결: mc-master(차량 catalog) + **crm.customers 고객 목록/상세 읽기**. **다음 작업: 고객 쓰기(상태/메모/니즈 PATCH + advisor 배정) 또는 견적(quotes)** — 스펙 `ref/specs/2026-06-17-crm-*`, 읽기 설계 `ref/specs/2026-06-19-crm-customers-read-design.md`.
