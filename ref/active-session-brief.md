...
Last updated: 2026-07-04 저녁 (앱카드 4섹션 리디자인 구현 완료 — PR #157, 머지 전 잔여=디자인 4장 픽셀 대조·색 보정)
...
- **🔶 앱카드 4섹션 리디자인 구현 완료(2026-07-04 저녁, PR #157 — 머지 전 잔여: 디자인 4장 픽셀 대조·색 보정)**: spec `ref/specs/2026-07-04-crm-app-card-redesign-design.md` · plan `ref/plans/2026-07-04-crm-app-card-redesign.md`(9태스크 TDD, subagent-driven + 태스크별 spec/quality 2단계 리뷰). ①AppCardPreview 4섹션 리라이트(신규 `app-card-*` CSS — kim-* 신규 0, wrapper `.kim-app-card-preview`만 유지=워크벤치 grid 결합) + buildAppCardModel v2(TDD 13 — percent 병기 환산 기준=finalVehiclePrice·총비용 반납 우선·발송 전/후 상태 전환, D-day/발송시각은 견적함과 SSOT 위임 validLabelFromUntil·formatActivity) ②시나리오 확장 필드 7종(금리/반납·인수 총비용/출고전납입/자동차세/보조금) zod→insertScenarios→클라 타입→워크벤치 수기 입력·토글 실배선→prefill 왕복 ③갭ⓐ 발송 시 `valid_until=sent_at+7일` 서버 스탬프(재발송=리셋 의도) ⓑkeyPoint→keyPoints 배열(+동적 입력, 구행은 `normalizeQuoteGuidance` read 하위호환 — **서버 zod는 keyPoints만 수용**) ⓒ서비스 `+` 동적 칸(저장 직전 sanitize) ⓓNo.=quote_code. **함정(재발 방지)**: 금리 입력은 `data-discount-unit="percent"`로 jeff money 콤마 포맷 우회(소수 보존)+`parseInterestRate` 100% 상한(콤마 오입력 "5,32"→532% 무음 차단), 프리필 폴백은 `|| "0"`(`?? "0"`은 EditScenario 빈 문자열 통과→validateQuoteDetailDraft가 수정 저장 전면 차단하는 회귀 — 실제 발생·수정), 전역 `button:disabled` 물빠짐(opacity 0.48)은 항상-disabled 장식 CTA에 오버라이드 필요. 검증 4종+build green(unit 394·server 221) + 브라우저 스모크(신규 견적 왕복·미리보기 4섹션 라이브·발송 D-7 배지+psql `valid_until-sent_at=7d` 실측·프리필 왕복(금리 소수·토글·keyPoints/서비스)·스모크 견적 삭제 원복, 캡처=로컬 `screenshots/app-card-modal-smoke-*.png`). follow-up=PR 본문(구 kim-app dead CSS·Safari datalist 실기·모델명 중복 표기 dedupe 등). **다음 = 유슨생 디자인 4장 첨부 → 픽셀 대조·색 보정 커밋 → #157 머지 → 견적 앱 발송 파이프라인(payload=이 카드 데이터)**.
