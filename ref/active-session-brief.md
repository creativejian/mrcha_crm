...
## Current Focus (2026-06-19) — 고객 쓰기 #2(자식 CRUD) 구현 완료(브랜치 `feat/crm-customer-write-children`, PR/머지 대기). 다음=#3 서류

- 인증+CF 배포 + 코드리뷰(#43~#45) + **CRM 고객 읽기(#46)** + **mc-master UX/성능(#47~#52)** + **고객 상세 읽기(#51)** + **라우팅 딥링크(#53)** + **로그인 로고(c213f14)** + **고객 쓰기 #1 본체(#54)** 전부 main 머지. **crm.mrcha.app 라이브**.
- **방금 완료(브랜치): 고객 쓰기 #2 — 자식 CRUD** — 메모/할일/일정 추가·수정·삭제·완료토글을 DB로(POST/PATCH/DELETE, 낙관+롤백, 임시 id→서버 uuid 교체). `customer_schedules.done` 신설(일정 완료 저장, drizzle 0002). 설계/플랜 `ref/{specs,plans}/2026-06-19-crm-customer-write-children*.md`.
- **다음 작업: 고객 쓰기 #3 — 서류(파일 업로드)** — `customer_documents` + 스토리지(Supabase Storage 등) 결정 필요. 이후 #4 견적, #5 advisor 배정. + (별도 설계 사이클) enum/lookup 도메인 정리.

## 완료 (2026-06-19)

- **고객 쓰기 #2 — 자식 CRUD (브랜치 `feat/crm-customer-write-children`, PR 대기)**: 메모/할일/일정 추가·수정·삭제·완료토글 DB 저장. 4커밋 — ①`customer_schedules.done` 컬럼(drizzle 0002, 일정 완료용). ②백 `customer-children.ts`(query 9: add/update/delete × memo/task/schedule, where=`id AND customer_id`) + `routes/customers.ts` 중첩 라우트 9개(`POST /:id/{memos,tasks,schedules}`·`PATCH·DELETE /:id/…/:childId`) + 서버테스트 3(라운드트립·404). ③프론트 `lib/customer-children.ts` 9개 + `CustomerDetailSchedule.done`. ④Kim 핸들러 12개 wiring(낙관+롤백, **추가 시 임시 id→서버 uuid 교체**, 토글=PATCH done, 일정 완료 시드 `detail.schedules.filter(done)`). **캐비엇**: 추가 직후 POST 해소 전(임시 id) 그 항목 조작은 `id.startsWith("kim-")` 가드로 API 생략(드문 race). **검증**: typecheck0·lint0·test:unit 125·test:server 39·build OK. 브라우저 유지확인 수동.
...
- ~~읽기 #51~~ · ~~딥링크 #53~~ · ~~쓰기 #1 본체 #54~~ · ~~쓰기 #2 자식 CRUD~~ ✅ 완료(브랜치 `feat/crm-customer-write-children`, PR 대기) — 위 "완료" 참고.
- **고객 쓰기 #3 — 서류(파일 업로드) (1순위)** — `customer_documents` 드롭/업로드/분류/순서/삭제. **스토리지 결정 선행**(Supabase Storage 버킷 등) + multipart. 현재 프론트는 objectURL 메모리만.
