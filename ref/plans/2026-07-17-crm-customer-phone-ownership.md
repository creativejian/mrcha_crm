...
- [x] T1 순수 전이 헬퍼(TDD — customer-phone.test.ts 11, RED 실관찰 후 GREEN)
- [x] T2 스키마+마이그 0034(실 DB 적용·psql 실측 — 컬럼·CHECK·제임스 백필 확인)
- [x] T3 서버 읽기 합성 + 쓰기 게이트 + 정규화(customers.phone.test.ts 5 — 합성 read-through 실 DB 실증)
- [x] T4 link SSOT 통합(applyAppUserLink) + 승격 phone 복사 제거(구 계약 테스트 4건 새 계약으로 갱신)
- [x] T5 클라(상세 잠금 토스트·추가 연락처 행·목록 병기·검색 포함·인박스 byPhone 제외 + 유닛 3)
- [x] T6 전체 검증 — typecheck 0 · lint 0 · unit 802 · server 575 · build (2026-07-17)
