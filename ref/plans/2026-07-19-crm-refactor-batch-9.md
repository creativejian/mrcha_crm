...
## 유슨생 판단 대기 (이번 배치 비이행 — 요청 시 후속)
- **A#2**(필드 집합 주석)·**A#3**(검색 빈 상태 3분기)·**A#4**(캡 카운트 표기)·**B-a**(wrapper 차단 1줄 폴리시)·**B-f**(normalizeName 파리티 유닛 — phone 쌍 동반 논점).

## 기록만 (재제안 금지 아님 — 재론 트리거 명시)
- **A#5** recent 삭제 고객 잔존(트리거 = 실사용 혼동 리포트) · **B-c** 공유 픽스처 리터럴(트리거 = 다음 서버 테스트 정비) · **B-g** 동명 교차 테스트(트리거 = name 매칭 로직 변경 시) · **B-h** 동명이인 판단 재료(트리거 = 실데이터 동명 등장).
