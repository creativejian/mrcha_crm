...
- **스모크 반영분(2026-07-02)**: AI 메시지 마크다운 렌더(C1 `MarkdownMessage` 재사용, 앱 미러 — 고객/상담원/system은 평문+pre-wrap 유지). `.message.ai .md-body`는 work-ai popover 룰셋을 말풍선 13px 기준 한 단계 축소 미러(본문13/h1 15/h2 14/h3 13.5, li::marker=brand, strong 600, 첫/마지막 블록 마진 제거). 타이핑 인디케이터 v1 편입(유슨생 지시) — 앱 `typing:<uid>` broadcast 상호운용(suffix 고유화 예외), 송신 1s leading-edge/수신 3s 타임아웃/고객 메시지 도착 시 즉시 클리어, 표시는 리스트 밖 입력창 위(앱 배치 미러, C1 `DoubleBounceDots` 14px 축소 재사용).
