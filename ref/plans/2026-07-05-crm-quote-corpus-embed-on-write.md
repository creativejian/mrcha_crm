...
- Task 4 확정 편차: 스펙의 "holdStreamLifetime 재사용+finally(release)" 대신 전용 `holdWork`(dbHold 체인) — 같은 요청 다건 스케줄(니즈 3필드 동시 PATCH)에서 덮어쓰기로 앞선 작업이 잘리는 문제 방지. `holdStreamLifetime`은 덮어쓰기 제약 주석 가드 추가(동작 불변).
- Task 6 리뷰 흡수: 게이트 sentinel 정규화(`trim().toLowerCase()` — 오타 fail-open 방지) + 게이트 skip **1회** 경고 로그(스펙 "skip(+로그)" 정합, 매 쓰기 로그는 노이즈라 once flag) + 프록시URL+무Authorization 동기 catch 유닛.
- **게이트 3규칙 강화(plan에 없던 커밋 abafab3)**: `bun test`(NODE_ENV=test 자동 설정)에서는 기본 off·명시적 `EMBED_ON_WRITE=on`만 허용. 배경 = T7 spec 리뷰 중 npm 스크립트(off 프리픽스) 우회 직접 `bun test <파일>` 실행이 실 Gemini 호출+master 임베딩 오염 14행을 실제로 냄(즉시 원복). bun이 NODE_ENV 미설정일 때만 자동 세팅하므로 best-effort — package.json 프리픽스가 1차 방어로 잔존.
- Task 5·7 테스트 픽스처 랜덤 서픽스(56c954e): 고정 customerCode/quoteCode가 afterAll 실패 시 unique 위반 연쇄 실패를 내는 공유 master 트랩 — `customer-quotes.send.test.ts` 관례로 통일. `embeddings.test.ts`의 기존 고정 픽스처(CU-EMBTEST-9990)는 슬라이스 이전 것이라 불변(follow-up).
- Task 8 리뷰 흡수(1de8149): 대표 시나리오 선택 규칙 3곳 복제 → `src/lib/primary-scenario.ts` `pickPrimaryScenario` SSOT 추출(발송 조립기·로더·백필 공유, asc 정렬 전제는 호출부 계약).
- Task 9 발견: `crm.assistant_messages.staff_user_id`는 **uuid 컬럼** — makeTestAuth 기본 sub `"test-user"`로는 /ask 영속이 실패한다. 스모크는 랜덤 UUID sub를 주입해 우회(향후 /ask 서버 테스트/스모크 동일 주의).
