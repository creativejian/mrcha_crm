...
**Architecture:** 고객 단위 니즈로 보고 `crm.customers`에 7개 nullable text 컬럼을 추가한다. 단일선택 닫힌집합 2개(연간주행·인도방식)는 DB CHECK + `client/src/data` 코드 상수 SSOT로 무결성을 보장하고, 나머지 5개(계약기간 다중·초기비용·태그 3)는 직렬화 text다. 쓰기는 기존 `savePatch`(PATCH /api/customers/:id) + `invalidateCustomerDetail` 인프라를 `needMethod` 패턴 그대로 재사용한다. (2026-06-30: 계약기간은 실무상 다중선택이 맞아 단일화 취소.)
...
| `src/db/schema.ts` | crm 스키마 정의 | customers 7컬럼 + 2 CHECK(계약기간 CHECK는 0010서 DROP) |
| `drizzle/0009_*.sql` · `0010_*.sql` | 마이그레이션 | 0009=7컬럼+3CHECK(적용), 0010=계약기간 CHECK DROP (crm only) |
...
| `src/routes/shared.ts` | DB 에러 메시지 | `dbErrorMessage` CHECK 위반 핸들러 |
...
| `client/src/components/customer-detail/hooks/useCustomerPurchase.ts` | 구매조건 상태·핸들러 | 초기화 9필드 + 7핸들러 savePatch (계약기간 다중 유지) |
| ~~`PurchaseConditions.tsx`~~ | 구매조건 렌더 | **변경 없음**(계약기간 다중 유지로 Task 6 취소) |
...
## Task 5: useCustomerPurchase 영속화 (초기화 + 7핸들러 savePatch, 계약기간 다중 유지)
...
- [ ] **Step 3: `togglePurchaseTerm`(다중 유지)에 savePatch 추가** (⚠️ 2026-06-30 결정: 계약기간은 다중선택 유지)

현재 `togglePurchaseTerm`(100-115행)에서 `nextValue` 계산 후 `setPurchaseFields` 직전 스냅샷, 끝에 savePatch 추가. 다중선택 Set 로직은 그대로. 변경 후 전체:
...
> 구매방식(`togglePurchaseMethod`)과 동일한 다중선택 패턴. popover는 닫지 않는다(연속 토글). 핸들러명·`renderPurchaseTermEditor` 그대로 → `PurchaseConditions.tsx` 변경 없음(Task 6 취소).
...
- [ ] **Step 10: (계약기간 다중 유지로 핸들러명 변경 없음 — `togglePurchaseTerm` 그대로 export)**

`return { ... handlers: { ... } }`의 `togglePurchaseTerm`은 이름 그대로 둔다(다중 유지 결정).
...
git commit -m "feat(crm): 구매조건 7필드 savePatch 영속 + 초기화 detail 매핑(계약기간 다중 유지)"
```

---

## Task 6: ~~PurchaseConditions.tsx 계약기간 단일 UI~~ — **취소 (2026-06-30)**

계약기간 다중선택 유지 결정으로 `PurchaseConditions.tsx`는 변경 불필요(`togglePurchaseTerm`·`renderPurchaseTermEditor` 다중 Set 로직 그대로). 이 Task는 수행하지 않는다.

---

## Task 2b: 계약기간 CHECK DROP(0010) + dbErrorMessage CHECK 핸들러

> Task 1(0009)이 `need_contract_term` CHECK를 포함해 적용됐는데, 계약기간 다중 결정으로 그 CHECK를 제거해야 한다(forward-only). 더해 Task 2 코드품질 리뷰의 Important(단일선택 CHECK 위반 시 raw 500 노출)도 함께 보정한다.

**Files:**
- Modify: `src/db/schema.ts` (needContractTerm CHECK 제거)
- Create: `drizzle/0010_*.sql` (db:generate 산출 — DROP CONSTRAINT)
- Modify: `src/routes/shared.ts` (`dbErrorMessage`에 CHECK 위반 핸들러)

- [ ] **Step 1: `schema.ts`에서 needContractTerm CHECK 제거**

`customers` 테이블 CHECK 배열에서 다음 한 줄을 삭제:

```ts
  check("customers_need_contract_term_check", inListCheck(t.needContractTerm, [...CONTRACT_TERM_OPTIONS, PURCHASE_UNSET_SENTINEL])),
```

남는 CHECK는 `need_annual_mileage`·`need_delivery_method` 2개. (`CONTRACT_TERM_OPTIONS` import는 purchase-meta가 쓰므로 유지 — schema.ts에서 더 안 쓰면 미사용 import가 되어 lint 경고가 날 수 있으니, schema.ts의 해당 import만 제거해야 하는지 typecheck/lint로 확인 후 정리.)

- [ ] **Step 2: typecheck**

Run: `bun run typecheck` — 0 errors. lint도 확인(`bun run lint`) — 미사용 import 0.

- [ ] **Step 3: 마이그레이션 생성**

Run: `bun run db:generate` — `drizzle/0010_*.sql` 생성. 내용은 `ALTER TABLE "crm"."customers" DROP CONSTRAINT "customers_need_contract_term_check";` 하나여야 한다. 다른 변경이 섞이면 STOP/BLOCKED.

- [ ] **Step 4: 마이그레이션 적용 (⚠️ master DB)**

Run: `bun run db:migrate` — 0010 적용. 이후 `need_contract_term`은 임의 text(다중 `·` 직렬화) 허용.

- [ ] **Step 5: `shared.ts` `dbErrorMessage`에 CHECK 위반 핸들러 추가**

`dbErrorMessage`(4행~)의 기존 핸들러들(foreign key, duplicate key) 사이/아래에 추가:

```ts
  if (/check constraint|23514/i.test(msg)) return "허용되지 않는 값입니다.";
```

위치는 `duplicate key` 핸들러 다음 줄. (단일선택 연간주행·인도방식 CHECK 위반 시 raw postgres 500 대신 친절 메시지. controlled UI라 실제 발생은 드묾.)

- [ ] **Step 6: 검증 + 커밋**

Run: `bun run typecheck && bun run lint && bun run test:server` — 전부 통과(test:server 125).

```bash
git add src/db/schema.ts src/routes/shared.ts drizzle/
git commit -m "fix(crm): 계약기간 CHECK 제거(0010, 다중 유지) + dbErrorMessage CHECK 위반 핸들러"
```
...
- [ ] **Step 2: psql로 CHECK 실측 (무결성 확인)**

Run(거부): `psql "$DATABASE_URL" -c "update crm.customers set need_annual_mileage='99만km' where customer_code='CU-2605-0020';"`
Expected: CHECK 위반 에러(`customers_need_annual_mileage_check`) — 사전 밖 값 거부.
Run(허용): `psql "$DATABASE_URL" -c "update crm.customers set need_contract_term='24개월 · 36개월' where customer_code='CU-2605-0020';"` → 성공(계약기간은 CHECK 없음, 다중 직렬화 허용). 확인 후 `set need_contract_term=null`로 복원.

- [ ] **Step 3: 브라우저 검증 (사용자 — 유슨생)**

`bun run dev` 후 김민준(CU-2605-0020) 상세 → 상세 구매조건:
- 7필드 각각 값 변경 → **새로고침 → 유지** 확인.
- 계약기간은 **다중선택**(여러 기간 토글, `"24개월 · 36개월"`)이 유지·저장되는지.
- 다중 태그 3개(계약 포커스·고객 특이사항·심사 특이사항)도 다중 선택·저장.
- 연간 주행거리·인도 방식 단일선택 저장.
- 구매방식·출고시기 회귀 0.
...
- **Spec coverage:** 7컬럼(Task1)·CHECK 2개(Task1 적용 3개 중 계약기간은 Task2b에서 DROP)·zod/Pick(Task2)·dbErrorMessage 핸들러(Task2b)·lib 4곳(Task3)·purchase-meta 맵(Task4)·7핸들러 savePatch+계약기간 다중 유지+초기화 매핑(Task5)·검증(Task7) — 전 항목 task 존재. Task6은 계약기간 다중 유지로 취소. `savePurchaseConditions`는 비활성 코드라 범위 밖.
- **Placeholder:** 없음(모든 step에 실제 코드/명령/기대출력).
- **Type 일관성:** `CustomerWritePatch`(lib·query 양쪽), `PURCHASE_FIELD_KEY`, `togglePurchaseTerm`(다중 유지), 컬럼명 camelCase↔snake_case 매핑 일관.
