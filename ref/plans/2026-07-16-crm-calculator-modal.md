...
- BNK 판매사 목록: 제프 **구현 완료 공유**(external 라우트·X-API-Key·에러 계약 calculate 동일·
  내부 무접촉) — 단 **prod 미배포**(CRM 실측 404, 대조군 calculate 정상 → 라우트 부재 확정).
  배포 확인 후 재실측(성공 shape `{ok:true,dealers}` vs `{dealers}` 확인) → UI 복원 반나절은
  🟡 이사님 수요 판단 대기(질의 framing: "제프 준비 완료 — 복원할까요").
