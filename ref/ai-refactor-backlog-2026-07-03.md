...
> **✅ 실행 결과(2026-07-03 밤)**: A 전체(1~5) = **PR #151 머지**, C 전체(7~10) = **PR #152 머지** + crm-analyst 재배포 완료. prod 스모크 통과(스트리밍 완주 첫바이트 2.79s·done 영속 + 조기 절단 시 부분 390자+" (중단됨)" 저장·유령 placeholder 0, 스모크 데이터 master에서 삭제). **6(sse-liveness M) = PR #153 머지(2026-07-04)** — TDD 유닛 8종(하트비트 3분기+raceDead) + 로컬 SSE 스모크(완주 done 영속·2s 절단 부분+"(중단됨)" 저장, 데이터 삭제). **잔여 = B 트리거 기반 3건뿐.**
...
## A. 즉시 착수 가능 (저위험 · S 위주 — 1~2 PR로 묶기 좋음) — ✅ 1~5 완료(#151)
...
### PR 후보 B — SSE 수명 구조화 (서버) — ✅ 4·5 완료(#151) · 6 완료(#153)
