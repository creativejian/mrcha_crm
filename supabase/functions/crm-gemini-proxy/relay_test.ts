...
Deno.test("퍼센트 인코딩(%2F 등) 포함 경로는 404 — allowlist 우회 차단", async () => {
  let called = false;
  const fetchImpl = (() => { called = true; return Promise.resolve(new Response("")); }) as unknown as typeof fetch;
  const res = await relayRequest(post("/v1beta/models/..%2F..%2Ffoo:generateContent"), fetchImpl);
  assertEquals(res.status, 404);
  assertEquals(called, false);
});

Deno.test("쿼리는 alt=sse만 전달 — 그 외 파라미터(key 등)는 제거", async () => {
  const urls: string[] = [];
  const fetchImpl = ((u: string | URL | Request) => { urls.push(String(u)); return Promise.resolve(new Response("{}")); }) as unknown as typeof fetch;
  await relayRequest(post("/v1beta/models/m:streamGenerateContent?key=LEAK&alt=sse&foo=1"), fetchImpl);
  await relayRequest(post("/v1beta/models/m:generateContent?key=LEAK"), fetchImpl);
  assertEquals(urls, [
...
Deno.test("업스트림 fetch 예외(네트워크 오류)는 502 JSON으로 변환", async () => {
  const fetchImpl = (() => Promise.reject(new TypeError("dns error"))) as unknown as typeof fetch;
