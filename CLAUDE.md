...
- 변경 후 검증: DOM/TS 변경은 `bun run typecheck`, 모든 변경은 `bun run lint`를 **0 problems**로 유지한다. 큰 변경은 `bun run build`, `bun run test:unit`까지 돌린다.
...
- `bun run db:push` 금지 — `catalog` schema의 차량 거울 테이블을 DROP할 수 있다. 스키마 변경은 `db:generate` → `db:migrate`만 사용한다. (차량 거울 상세: `ref/vehicle-mirror-db.md`)
