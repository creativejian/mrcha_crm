...
- **CI(2026-07-22 도입, `.github/workflows/ci.yml`)**: PR과 main push마다 **typecheck·lint·test:unit·build**가 자동으로 돈다. 로컬 검증을 건너뛰라는 뜻은 아니다(CI는 마지막 그물이지 1차 방어선이 아니다). ⚠️ `test:server`는 공유 master DB에 붙어 운영 알림까지 발사하므로 **CI에 넣지 않는다** — 로컬 전용이다.
