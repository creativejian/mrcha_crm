...
- **CI(2026-07-22 도입, `.github/workflows/ci.yml`)**: PR과 main push마다 **7단계**가 자동으로 돈다 — `typecheck` · `lint` · **`knip`** · **`format:check`** · `test:unit` · `build` · **`test:edge`**(Deno). 로컬 검증을 건너뛰라는 뜻은 아니다(CI는 마지막 그물이지 1차 방어선이 아니다).
  - ⚠️ **잡 이름은 `typecheck · lint · unit · build`라 4종만 말한다 — 실제로는 위 7단계다.** `gh pr checks` 출력도 그 이름이라 knip·format:check·edge가 안 보인다. 실제 목록은 `ci.yml`의 steps로 확인할 것. (2026-07-23 #333: 그 이름과 당시 스테일했던 AGENTS.md 서술을 믿고 로컬 knip을 건너뛰었다가 unused export 1건으로 CI가 빨개졌다.)
